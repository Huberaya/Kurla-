...
|---|----------|--------|--------------------------------|:-----:|:-----:|:-------:|
| **G1** | Base réelle intégrée | Exécuter `npm run test:realdb` (preflight + intégrations + contrats de schéma) sur le projet prod ; migration 83/83 appliquées ; RLS A/B (2 comptes, tentative d'accès croisé) | 6 bancs réel-DB verts contre la base prod ; 0 migration en retard ; capture d'écran du rapport A/B en `docs/` | 2 | 1 | **PARTIEL ✅** (2026-09-12) : 0 migration en retard (diff local/remote + audit objets, 33 objets backfillés) ; A/B RLS vert sur données réelles (`docs/GO_LIVE_RLS_AB_REPORT.md`, 4/4 assertions + flux suppression RGPD validé en prod) ; 6 bancs `test:realdb` **⏳ bloqués sur `SUPABASE_SERVICE_ROLE_KEY`** (la clé de management `vcp_`/`sbp_` est 401 GoTrue admin — définitif) |
