...
export const PeauJ3J7WhitecastLotPanel: React.FC<{ headers: HeadersInit }> = ({ headers }) => {
  ...
  return (
    <div className="rounded-3xl bg-[#1A0F0A] border border-sky-500/20 p-6 sm:p-8 space-y-6 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-[#FFF7EF]/60 mt-1 max-w-3xl leading-relaxed">
            <strong className="text-[#FFF7EF]">J+3 relance</strong> si silence (emailed sans réponse) → <strong className="text-sky-300">J+7 bilan &gt;30% (2/5) + 1 whitecast faible</strong> → <strong className="text-emerald-300">J+10 échantillon</strong> photo V–VI lumière du jour → <strong className="text-[#D49A63]">J+14 lot 50</strong> si whitecast faible + PIF/CPSR/CPNP fichier+date. Une case cochée sans preuve fichier+date ne passe pas le gate.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 rounded-2xl bg-[#050403] border border-amber-500/20 text-center"><p className="text-[10px] uppercase tracking-wider font-bold text-amber-300">À relancer J+3</p><p className="text-xl font-bold text-amber-300">{needRelance}</p><p className="text-[10px] text-[#FFF7EF]/40">emailed sans relance</p></div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-sky-500/20 text-center"><p className="text-[10px] uppercase tracking-wider font-bold text-sky-300">Réponses J+7</p><p className="text-xl font-bold text-sky-300">{counts.replied}/5</p><p className="text-[10px] text-[#FFF7EF]/40">cible ≥2</p></div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-emerald-500/20 text-center"><p className="text-[10px] uppercase tracking-wider font-bold text-emerald-300">Whitecast faible</p><p className="text-xl font-bold text-emerald-300">{counts.whitecastOk}/5</p><p className="text-[10px] text-[#FFF7EF]/40">échantillon validé</p></div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-[#C8753D]/30 text-center"><p className="text-[10px] uppercase tracking-wider font-bold text-[#C8753D]">Lots 50 prêts</p><p className="text-xl font-bold text-[#C8753D]">{counts.lotReady}/5</p><p className="text-[10px] text-[#FFF7EF]/40">whitecast faible + qty≥50</p></div>
      </div>
      ...
      <div className="space-y-3">
        {J0_IDS.map(id=>{
          ...
          return (
            <div key={id} className="rounded-2xl border border-[#FFF7EF]/10 bg-[#050403] p-4 space-y-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-bold text-sm text-[#FFF7EF]">{id}</span>
                    <span className={`px-2 py-0.5 rounded-full border text-[10px] font-bold ${status==='to_contact' ? 'bg-[#FFF7EF]/5 text-[#FFF7EF]/50 border-[#FFF7EF]/10' : status==='emailed' ? 'bg-sky-500/10 text-sky-300 border-sky-500/30' : status==='replied' ? 'bg-cyan-500/10 text-cyan-300 border-cyan-500/30' : 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30'}`}>{status}</span>
                    {prospect?.updatedAt && <span className="text-[10px] text-[#FFF7EF]/40">maj {new Date(prospect.updatedAt).toLocaleDateString('fr-FR')}</span>}
                  </div>
                  <p className="text-[11px] text-[#FFF7EF]/55 mt-1">Relance J+3 si emailed sans réponse → whitecast J+10 → lot 50 J+14 si PIF/CPSR/CPNP fichier+date</p>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  <button onClick={()=>copy(id+'-relance', RELANCE_TEMPLATE(id))} disabled={!needsRelance}
                    className={`px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1 ${needsRelance ? 'bg-sky-600 hover:bg-sky-500 text-white' : 'bg-[#FFF7EF]/10 text-[#FFF7EF]/30 cursor-not-allowed'}`}>
                    {copiedId===id+'-relance' ? <Check className="w-3.5 h-3.5" /> : <Send className="w-3.5 h-3.5" />} {copiedId===id+'-relance'?'Copié':'Copier relance J+3'}
                  </button>
                  <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#1A0F0A] border border-[#FFF7EF]/15 text-xs cursor-pointer">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <label className="space-y-1">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-[#D49A63] flex items-center gap-1"><Eye className="w-3 h-3" /> Whitecast V–VI (lumière du jour)</span>
                  <select value={ex.whitecast||''} onChange={e=>updateExtra(id,{ whitecast:e.target.value as any })}
                    className="w-full px-3 py-2 rounded-xl bg-[#1A0F0A] border border-[#FFF7EF]/15 text-xs text-[#FFF7EF] focus:outline-none focus:border-[#C8753D]">
                  ...
                  <span className={`text-[11px] ${ex.whitecast==='faible'?'text-emerald-300':ex.whitecast==='eleve'?'text-rose-300':'text-[#FFF7EF]/50'}`}>
                ...
                <label className="space-y-1">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-[#D49A63] flex items-center gap-1"><Beaker className="w-3 h-3" /> Échantillon validé</span>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#FFF7EF]/70">{ex.sampleValidated ? 'Kit reçu + test OK' : 'En attente kit'}</span>
                  </div>
                  <p className="text-[10px] text-[#FFF7EF]/40">1 kit complet avant lot — pas de lot sans échantillon</p>
                </label>

                <label className="space-y-1">
                  <span className="text-[10px] uppercase tracking-wider font-bold text-[#D49A63] flex items-center gap-1"><Package className="w-3 h-3" /> Lot à commander</span>
                  <div className="flex items-center gap-2">
                    <input type="number" min={0} step={10} value={ex.lotQty||''} onChange={e=>updateExtra(id,{ lotQty:e.target.value })}
                      placeholder="50" className="w-20 px-3 py-2 rounded-xl bg-[#1A0F0A] border border-[#FFF7EF]/15 text-xs text-[#FFF7EF] font-mono focus:outline-none focus:border-[#C8753D]" />
                    <span className="text-xs text-[#FFF7EF]/50">unités</span>
                  ...
                  <p className="text-[10px] text-[#FFF7EF]/40">MOQ 50–100 — lot créé dans <code className="px-1 py-0.5 rounded bg-[#1A0F0A] border border-[#FFF7EF]/10">Lots & traçabilité</code></p>
                </label>
              </div>

              <div className="flex flex-wrap gap-2 text-[11px]">
                <span className={`px-2 py-1 rounded-full border text-[10px] font-bold ${isEmailed?'bg-sky-500/10 text-sky-300 border-sky-500/20':'bg-[#FFF7EF]/5 text-[#FFF7EF]/40 border-[#FFF7EF]/10'}`}>J0 mailed {isEmailed?'✓':''}</span>
                <span className={`px-2 py-1 rounded-full border text-[10px] font-bold ${ex.relanceSent?'bg-emerald-500/10 text-emerald-300 border-emerald-500/20':'bg-[#FFF7EF]/5 text-[#FFF7EF]/40 border-[#FFF7EF]/10'}`}>J+3 relance {ex.relanceSent?'✓':''}</span>
                <span className={`px-2 py-1 rounded-full border text-[10px] font-bold ${ex.whitecast==='faible'&&ex.sampleValidated?'bg-emerald-500/10 text-emerald-300 border-emerald-500/20':'bg-[#FFF7EF]/5 text-[#FFF7EF]/40 border-[#FFF7EF]/10'}`}>J+10 échantillon {ex.whitecast==='faible'&&ex.sampleValidated?'✓ whitecast faible':''}</span>
                <span className={`px-2 py-1 rounded-full border text-[10px] font-bold ${ex.lotQty&&Number(ex.lotQty)>=50&&ex.whitecast==='faible'&&ex.sampleValidated?'bg-emerald-500/10 text-emerald-300 border-emerald-500/20':'bg-[#FFF7EF]/5 text-[#FFF7EF]/40 border-[#FFF7EF]/10'}`}>J+14 lot 50 {ex.lotQty&&Number(ex.lotQty)>=50?'✓':''}</span>
                <a href="#batches" className="ml-auto px-3 py-1 rounded-full bg-[#C8753D]/20 border border-[#C8753D]/30 text-[#D49A63] text-xs font-bold flex items-center gap-1"><Truck className="w-3 h-3" /> Créer lot →</a>
              </div>
            </div>
          );
        })}
      ...
      <p className="text-[10px] text-[#FFF7EF]/35 text-center leading-relaxed">C22 P2 — suivi <span className="text-[#D49A63]">localStorage {`kurla_c22p2_whitecast_lot`}</span> + live <span className="text-[#D49A63]">/api/admin/sourcing/prospects</span> (5 J0). Relance J+3, bilan J+7, échantillon J+10 lumière du jour, lot 50 J+14 uniquement si whitecast faible + PIF/CPSR/CPNP. Aucune commande avant gate vert.</p>
    </div>
  );
};
