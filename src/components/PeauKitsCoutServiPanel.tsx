    ...
    <div className="rounded-3xl bg-[#1A0F0A] border border-[#FFF7EF]/10 p-6 sm:p-8 space-y-6 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-lg font-serif-title font-bold text-[#FFF7EF] flex items-center gap-2">
            <Calculator className="w-5 h-5 text-[#C8753D]" /> C17 — Kits peau : coût servi vs prix public
            <span className="px-2 py-0.5 rounded-full bg-[#C8753D]/15 border border-[#C8753D]/30 text-[#D49A63] text-[10px] font-bold">lot → kit → commande</span>
          </h3>
          <p className="text-xs text-[#FFF7EF]/60 mt-1 max-w-3xl leading-relaxed">
            Chaque kit est un <strong className="text-[#FFF7EF]">assemblage</strong> : son coût servi = somme des coûts servis de ses composants (lots reçus). Tant qu’aucun lot peau n’est réceptionné, seules les <strong className="text-[#D49A63]">cibles HT 22 / 30 / 40€</strong> s’affichent — la marge réelle se calcule sur du réel, jamais sur une estimation.
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {PEAU_KITS.map(kit=>{
          ...
          return (
            <div key={kit.id} className="rounded-2xl bg-[#050403] border border-[#FFF7EF]/10 p-4 space-y-3 flex flex-col">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[11px] uppercase tracking-wider font-bold text-[#D49A63]">{kit.id} · {kit.tier}</p>
                  <h4 className="text-sm font-bold text-[#FFF7EF] leading-tight">{kit.name}</h4>
                  <p className="text-[11px] text-[#FFF7EF]/55">{kit.tagline} · {kit.products.length} soins</p>
                </div>
                <span className={`px-2 py-1 rounded-full border text-xs font-bold ${kit.id==='KPEAU-02'?'bg-emerald-500/15 text-emerald-300 border-emerald-500/30':'bg-[#FFF7EF]/5 text-[#FFF7EF]/70 border-[#FFF7EF]/10'}`}>{kit.priceBundle.toFixed(2)}€</span>
              </div>
                ...
                {components.map(c=>{
                  ...
                  return (
                    <div key={c.id} className="flex items-center justify-between text-xs gap-2">
                      <span className="text-[#FFF7EF]/80 leading-tight flex-1">{c.name}<span className="text-[#FFF7EF]/40"> · {c.role}</span></span>
                      <span className="text-[10px] font-mono shrink-0">
                        {cost!==null ? <span className="text-emerald-300">{(cost/100).toFixed(2)}€ {lots.length>1&&`· ${lots.length} lots`}</span>
                        : <span className="text-[#FFF7EF]/40">aucun lot</span>}
                      </span>
                    </div>
                  );
                })}
              ...
              <div className="mt-auto space-y-2 pt-3 border-t border-[#FFF7EF]/10">
                <div className="flex justify-between text-xs">
                  <span className="text-[#FFF7EF]/60">Prix public</span><span className="font-bold text-[#FFF7EF]">{prixTTC.toFixed(2)}€ <span className="font-normal text-[#FFF7EF]/40">éco {kit.economy.toFixed(2)}€ −{kit.economyPct}%</span></span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#FFF7EF]/60 flex items-center gap-1"><Boxes className="w-3 h-3" /> Coût servi kit</span>
                  {sumCost!==null
                ...
                {marge!==null && <div className="flex justify-between text-xs"><span className="text-[#FFF7EF]/60 flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Marge réelle</span><span className={`font-bold ${marge>=52?'text-emerald-300':marge>=40?'text-amber-300':'text-rose-300'}`}>{marge.toFixed(0)}%</span></div>}
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                <span className="px-2 py-1 rounded-full bg-[#1A0F0A] border border-[#FFF7EF]/10 text-[10px] text-[#FFF7EF]/50">{kit.routine}</span>
      ...
      <div className="p-3 rounded-2xl bg-sky-950/20 border border-sky-500/20 text-xs text-sky-200 flex gap-2">
        ...
        <span><strong>Traçabilité :</strong> un <code className="px-1 py-0.5 rounded bg-[#050403] border border-[#FFF7EF]/10 font-mono text-[11px]">lot X</code> alloué à une ligne de commande = réponse à « quelles commandes contiennent le lot X » en 1 requête (<code>batch_order_trace</code>). Les kits ne sont jamais alloués tels quels : ce sont leurs composants qui portent les lots — la traçabilité reste unitaire.</span>
      </div>

      <p className="text-[10px] text-[#FFF7EF]/35 text-center leading-relaxed">C17 — source prix : <span className="text-[#D49A63]">peauKits.ts</span> (49,70/62/84,90€, économies 2,90/9,40/14,90€) + <span className="text-[#D49A63]">cahier_des_charges_peau.md</span> (cibles &lt;22/30/40€ HT) + lots réels <span className="text-[#D49A63]">/api/admin/batches</span> + double sourcing. Aucune valeur estimée affichée comme réelle.</p>
    </div>
  );
};
