    ...
    <div className="rounded-3xl bg-kurla-espresso border border-kurla-cream/10 p-6 sm:p-8 space-y-7 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-lg font-serif-title font-bold text-kurla-cream flex items-center gap-2">
            <Crown className="w-5 h-5 text-kurla-copper" /> C25 — Croissance + Contenu SEO + IA mélanine + Monitoring (tout)
          ...
          <p className="text-xs text-kurla-cream/60 mt-1 max-w-3xl leading-relaxed">
            Dernier étage avant scale : <strong className="text-kurla-cream">1 000 clients M6 (AOV 52€ kit)</strong> + <strong className="text-kurla-cream">15 fiches peau V-VI safe</strong> + <strong className="text-kurla-cream">IA gardes mélanine</strong> (uniformiser≠éclaircir, HPI, whitecast) + <strong className="text-kurla-cream">monitoring live</strong>. Tout en TEST reste honnête — un vert n'arrive que sur métrique réelle, pas estimée.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-[10px] uppercase tracking-wider text-kurla-amber font-bold flex items-center justify-center gap-1"><Users className="w-3 h-3" /> Growth</p>
          <p className="text-xl font-bold text-kurla-cream">{waitlist}<span className="text-xs font-normal opacity-60"> waitlist</span> · {paid} paid</p>
          <p className="text-[10px] text-kurla-cream/40">AOV {aov}€ · {referralOrders} parrainages</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          ...
          <p className="text-[10px] text-kurla-cream/40">{contents.length} contenus totaux</p>
        </div>
        <div className={`p-3 rounded-2xl border text-center ${health?.geminiEnabled?'bg-emerald-500/10 border-emerald-500/20':'bg-amber-500/10 border-amber-500/20'}`}>
          ...
          <p className="text-[10px] text-kurla-cream/40">{zeroSearch} recherches sans résultat</p>
        </div>
        <div className={`p-3 rounded-2xl border text-center ${emailHealth?.outage?'bg-rose-500/10 border-rose-500/20':'bg-indigo-500/10 border-indigo-500/20'}`}>
          ...
          <p className="text-[10px] text-kurla-cream/40">FR82/BE76 · {gatesOk}/{gatesTotal} gates</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5 text-[10px]">
        ...
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><Wallet className="w-3 h-3" /> AOV cible 52€ kit</span>
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Uniformiser≠éclaircir</span>
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><Globe className="w-3 h-3" /> FR82/BE76 LIVE si sk_live</span>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="p-4 rounded-2xl bg-kurla-ink border border-kurla-copper/25 space-y-3">
          <h4 className="text-sm font-bold text-kurla-cream flex items-center gap-2"><Megaphone className="w-4 h-4 text-kurla-copper" /> Growth — 1 000 clients M6 (AOV 52€)</h4>
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Waitlist</p><p className="text-lg font-bold text-kurla-cream">{waitlist}</p><p className="text-[10px] text-kurla-cream/35">launch_leads</p></div>
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Paid</p><p className="text-lg font-bold text-emerald-300">{paid}</p><p className="text-[10px] text-kurla-cream/35">orders</p></div>
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Parrainage</p><p className="text-lg font-bold text-sky-300">{referralOrders}</p><p className="text-[10px] text-kurla-cream/35">récompenses</p></div>
          </div>
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="text-xs font-bold text-kurla-cream flex items-center gap-1"><BarChart3 className="w-3.5 h-3.5" /> Entonnoir {funnel ? '' : '(pas encore branché)'}</p>
            {funnel?.stages ? (
              <div className="mt-2 space-y-1.5">
                {funnel.stages.slice(0,5).map((s:any)=>(
                  <div key={s.key} className="flex justify-between text-xs"><span className="text-kurla-cream/70">{s.label}</span><span className="font-mono font-bold text-kurla-cream">{s.value}</span></div>
                ))}
                <div className="flex gap-2 text-[11px] mt-2 flex-wrap">
                  <span className="px-2 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60">cart→order {funnel.conversions?.cartToOrderPct ?? '—'}%</span>
                  <span className="px-2 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60">kit {funnel.conversions?.kitSharePct ?? '—'}%</span>
                  <span className="px-2 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60">repeat {funnel.conversions?.repeatRatePct ?? '—'}%</span>
                </div>
              </div>
            ) : (
              <p className="text-[11px] text-kurla-cream/50 mt-1 leading-relaxed">Funnel branché côté strategy cockpit (<code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">/api/admin/strategy/cockpit</code>) — affiche ventes réelles par canal (attribution UTM), pas d'estimation. Tant que 0 commande, tout est à 0 (honnête).</p>
            )}
          </div>
          <p className="text-[11px] text-kurla-cream/40 leading-relaxed">Canal pilote FR82 (3,2M textured) → BE76 J+30 → SN71 J+60. Budget test 360€ échantillons + 4 000€/mois plein régime. Objectif : CAC &lt; AOV × marge 52%.</p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-kurla-ink border border-sky-500/20 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2 rounded-xl bg-sky-950/20 border border-sky-500/20"><p className="text-[10px] uppercase tracking-wider text-sky-200/70">Fiches peau</p><p className="text-lg font-bold text-sky-300">{contentPeau}/15</p><p className="text-[10px] text-kurla-cream/35">HPI/SPF/barrière</p></div>
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Guides</p><p className="text-lg font-bold text-kurla-cream">7 essais</p><p className="text-[10px] text-kurla-cream/35">HPI/SPF/textures</p></div>
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Total CMS</p><p className="text-lg font-bold text-kurla-cream">{contents.length}</p><p className="text-[10px] text-kurla-cream/35">articles</p></div>
          </div>
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="text-xs font-bold text-kurla-cream">Actifs couverts (8) — source <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">skinTaxonomy</code></p>
            ...
            <p className="text-[11px] text-kurla-cream/40 mt-2 leading-relaxed">Chaque fiche = INCI + preuve V-VI safe + CosIng → <code>/ingredient/:id</code> + visuel whitecast. RTBF : aucune allégation « éclaircit » — vocabulaire <strong className="text-kurla-cream">uniformiser</strong>.</p>
          </div>
          <div className="flex gap-1.5 flex-wrap">
            ...
            <a href="/ingredients" target="_blank" rel="noreferrer" className="px-2.5 py-1 rounded-full bg-kurla-espresso border border-kurla-cream/10 text-xs text-kurla-cream/70">Ingrédients</a>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-kurla-ink border border-emerald-500/20 space-y-3">
          ...
          <div className="p-3 rounded-xl bg-emerald-950/15 border border-emerald-500/15">
            ...
            <ul className="mt-2 space-y-1 text-[11px] text-kurla-cream/70 leading-relaxed list-disc list-inside">
              <li><strong className="text-kurla-cream">uniformiser ≠ éclaircir</strong> — 0 occurrence éclaircir/blanchir (grep 0)</li>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10 text-center"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Gemini</p><p className={`text-sm font-bold ${health?.geminiEnabled?'text-emerald-300':'text-amber-300'}`}>{health?.geminiEnabled?'ON':'OFF'}</p><p className="text-[10px] text-kurla-cream/35">{health?.geminiEnabled?'assistant live':'fallback'}</p></div>
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10 text-center"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Usage IA</p><p className="text-sm font-bold text-kurla-cream">{aiRate}</p><p className="text-[10px] text-kurla-cream/35">profils actifs</p></div>
          </div>
          <p className="text-[11px] text-kurla-cream/40 leading-relaxed">Sources : <code className="px-1 py-0.5 rounded bg-kurla-espresso border border-kurla-cream/10">/api/health</code> (gemini) + <code>/api/admin/metrics</code> (aiUsageRate, zeroSearch {zeroSearch}) + <code>src/lib/ai/assistant.ts</code> (medicalTriage). Aucun produit demo — l'IA ne cite que le catalogue vérifié.</p>
          <div className="flex gap-1.5">
            ...
            <span className="text-[10px] text-kurla-cream/35 self-center">Prompt test : « HPI post-bouton 40€ sans parfum »</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-kurla-ink border border-indigo-500/20 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-center">
            ...
            <div className="p-2 rounded-xl bg-kurla-espresso border border-kurla-cream/10"><p className="text-[10px] uppercase tracking-wider text-kurla-cream/50">Demande</p><p className="text-sm font-bold text-kurla-cream">{metrics?.totalOrders ?? 0} cmd</p><p className="text-[10px] text-kurla-cream/35">préco 3–5j</p></div>
          </div>
          <div className={`p-3 rounded-xl border flex gap-2 ${emailHealth?.outage||!emailHealth?.isRealProvider?'bg-rose-950/20 border-rose-500/20':'bg-emerald-950/15 border-emerald-500/20'}`}>
            ...
            <div className="text-xs leading-relaxed">
              ...
              <p className="text-kurla-cream/60">{emailHealth?.what || 'Aucune panne détectée'} — provider <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">{emailHealth?.provider||'console'}</code> {emailHealth?.counts ? `· ${emailHealth.counts.failed} fail / ${emailHealth.counts.sent} sent` : ''}</p>
            </div>
          </div>
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="text-xs font-bold text-kurla-cream flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> Règle d'or monitoring</p>
            <p className="text-[11px] text-kurla-cream/60 leading-relaxed mt-1">Un gate rouge = commande bloquée (voulu). Un pays &lt;65 (CH 58/CM 52) = TEST forcé même si sk_live. Aucune liste n'est inventée : ce qui n'est pas reçu s'affiche bloqué.</p>
          </div>
          <p className="text-[11px] text-kurla-cream/40 leading-relaxed">Sources : <code className="px-1 py-0.5 rounded bg-kurla-espresso border border-kurla-cream/10">/api/admin/operations/cockpit</code> + <code>/api/admin/metrics</code> + <code>/api/admin/email-health</code> + <code>/api/health</code></p>
        </div>
      </div>

      <div className="flex flex-wrap gap-2">
        <button onClick={()=>copy(goChecklist,'c25')} className="px-3 py-1.5 rounded-full bg-kurla-copper hover:bg-kurla-amber text-white text-xs font-bold flex items-center gap-1.5">
        ...
        <a href="/peau/journal" target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-full bg-kurla-ink border border-kurla-cream/15 text-xs text-kurla-cream/70">Journal peau</a>
        <span className="text-[11px] text-kurla-cream/35 self-center">C25 = P1 croissance + P1 contenu + P1 IA + P1 monitoring — scale seulement après GO 65/100.</span>
      </div>
      ...
      <p className="text-[10px] text-kurla-cream/35 text-center leading-relaxed">C25 — sources : <span className="text-kurla-amber">/api/admin/metrics</span> (waitlist, paid, aov, aiRate) + <span className="text-kurla-amber">/api/admin/strategy/cockpit</span> (funnel, referral, channels) + <span className="text-kurla-amber">/api/admin/operations/cockpit</span> (gates) + <span className="text-kurla-amber">/api/content</span> (fiches) + <span className="text-kurla-amber">/api/health</span> (gemini) + <span className="text-kurla-amber">/api/admin/email-health</span>.</p>
    </div>
  );
};
