        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
          <Reveal>
            <div className="max-w-[600px]">
              ...
              <p className="text-sm sm:text-base text-kurla-carbon/75 font-light leading-relaxed">
                Nous repérons des marques indépendantes — fondées par et pour des peaux et cheveux texturés — pour les rassembler sur KURLA. Voici les marques que nous aimons et qui arrivent dans la boutique. <span className="font-medium text-kurla-carbon">Vous voulez vendre votre marque sur KURLA ?</span>
              </p>
            </div>
          </Reveal>
          ...
          <Reveal delay={0.2}>
            <div className="flex flex-wrap gap-3">
              <a
                href="/marques"
                className="px-6 py-3.5 rounded-full bg-kurla-copper hover:bg-kurla-cocoa text-white text-xs font-semibold tracking-wide transition-all flex items-center gap-2 shadow-md shrink-0"
              >
                <Store className="w-4 h-4" /> Devenir marque partenaire
              </a>
              <a
                href="/community"
                className="px-6 py-3.5 rounded-full bg-white border border-kurla-stone hover:border-kurla-copper text-kurla-carbon text-xs font-semibold tracking-wide transition-all flex items-center gap-2 shrink-0"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {BRANDS_LIST.map((b, idx) => (
            <Reveal key={b.id} delay={0.08 * idx}>
              <div className="group rounded-3xl bg-white border border-kurla-stone hover:border-kurla-copper p-6 shadow-sm hover:shadow-xl transition-all h-full flex flex-col">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <h3 className="text-xl font-serif-title font-bold text-kurla-carbon group-hover:text-kurla-copper transition-colors">
                      {b.name}
                    </h3>
                    <span className="mt-1 inline-flex items-center gap-1 text-[11px] font-medium text-kurla-carbon/60">
                      <MapPin className="w-3 h-3" /> {b.country}
                    </span>
                  </div>
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-kurla-sand text-[10px] font-semibold text-kurla-copper border border-kurla-copper/20 whitespace-nowrap">
                    <Sparkles className="w-3 h-3" /> À venir
                  </span>
                </div>

                <span className="self-start text-[11px] font-semibold text-kurla-copper bg-kurla-sand px-2.5 py-1 rounded-full mb-3">
                  {b.category}
                </span>

                <p className="text-xs text-kurla-carbon/75 font-light leading-relaxed flex-1">
                  {b.tagline}
                </p>

                <div className="pt-4 mt-4 border-t border-kurla-stone flex items-center justify-between">
                  ...
                  <a
                    href="/community"
                    className="text-xs font-bold text-kurla-copper hover:text-kurla-cocoa flex items-center gap-1"
                  >
