    ...
    <div className="rounded-3xl bg-kurla-espresso border border-kurla-cream/10 p-6 sm:p-8 space-y-6 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-kurla-cream/60 mt-1 max-w-3xl leading-relaxed">
            Parcours bout-en-bout sans dead-end, sans claim médical, <strong className="text-kurla-cream">uniformiser≠éclaircir</strong> vérifié (grep 0 éclaircir), SPF naturel 13≠50, rétinol×AHA alterner, V-VI safe. Préco lun/jeu 18h 0 stock Paris, Stripe TEST, 8 gates fichier+date — un gate rouge = commande bloquée (voulu).
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-[10px] uppercase tracking-wider text-kurla-amber font-bold">Readiness</p>
          <p className="text-xl font-bold text-kurla-cream">{live?.readiness ?? '—'}</p>
          <p className="text-[10px] text-kurla-cream/40">perProduct</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-[10px] uppercase tracking-wider text-sky-300 font-bold">Lots</p>
          <p className="text-xl font-bold text-kurla-cream">{live?.batches ?? '—'}</p>
          <p className="text-[10px] text-kurla-cream/40">batches reçus</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-[10px] uppercase tracking-wider text-amber-300 font-bold">Demande</p>
          <p className="text-xl font-bold text-kurla-cream">{live?.demand ?? '—'}</p>
          <p className="text-[10px] text-kurla-cream/40">products à sourcer</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-[10px] uppercase tracking-wider text-emerald-300 font-bold">Fournisseurs</p>
          <p className="text-xl font-bold text-kurla-cream">{live?.suppliers ?? '—'}</p>
          <p className="text-[10px] text-kurla-cream/40">référentiel</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-kurla-cream/10">
        <table className="w-full text-left text-xs border-collapse">
          ...
          <tbody className="divide-y divide-kurla-cream/5">
            {STEPS.map(s=> (
              <tr key={s.n} className="hover:bg-kurla-cream/[0.02]">
                <td className="px-3 py-2 font-mono text-kurla-cream/40">{s.n}</td>
                <td className="px-3 py-2 text-kurla-cream/80 leading-snug">{s.label}</td>
      ...
      <div className="flex flex-wrap gap-2 text-[11px]">
        <span className="px-3 py-1.5 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1.5"><Package className="w-3.5 h-3.5" /> Boutique filtre peau ?taches&sansParfum&budget&phototype=V → hub honnête si 0 publié</span>
        <span className="px-3 py-1.5 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1.5"><Truck className="w-3.5 h-3.5" /> 3–5j lun/jeu 18h · FR82/BE76 LIVE si sk_live</span>
      ...
      <p className="text-[10px] text-kurla-cream/35 text-center leading-relaxed">C21 — doc complète : <span className="text-kurla-amber">QA_FATOU_C21.md</span> (16 étapes Fatou + cockpit C16–C20 consolidé) + live QA ci-dessus. Build 7.70s admin 639kB. P1 : 5 mails J0 → 3 échantillons → 1 lot → gate vert → annuaire peau 1 pro.</p>
    </div>
  );
};
