...
const inputClass = 'w-full px-3 py-2.5 rounded-xl bg-kurla-ink border border-kurla-cream/10 text-kurla-cream text-xs placeholder:text-kurla-cream/30 focus:outline-none focus:border-kurla-copper';
const buttonClass = 'inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-kurla-copper hover:bg-kurla-amber text-white text-xs font-semibold disabled:opacity-40';
const mutedButtonClass = 'inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-kurla-ink border border-kurla-cream/10 hover:border-kurla-copper/60 text-kurla-cream/80 text-xs font-semibold disabled:opacity-40';
...
export const AdminOperationsPanel: React.FC<Props> = ({ dashboard, headers, onReload }) => {
  ...
  const renderEmpty = (text: string) => <p className="text-xs text-kurla-cream/45 italic">{text}</p>;

  const renderBrands = () => (
    <div className="grid lg:grid-cols-[1fr_360px] gap-6">
      <div className="space-y-3">
        {rows.brands?.length ? rows.brands.filter((brand: any) => matches(brand.name, brand.description)).map((brand: any) => <div key={brand.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 flex items-center justify-between gap-3">
          <div><p className="text-sm font-semibold">{brand.name}</p><p className="text-[11px] text-kurla-cream/45">{brand.description || 'Description non renseignée.'}</p></div>
      ...
      <form className="p-5 rounded-2xl bg-kurla-ink border border-kurla-cream/10 space-y-3" onSubmit={e => { e.preventDefault(); saveEntity('brand', brandForm, editingBrand?.id); }}>
        <h3 className="text-sm font-semibold flex items-center gap-2"><Plus className="w-4 h-4 text-kurla-amber" /> {editingBrand ? 'Modifier la marque' : 'Ajouter une marque'}</h3>
  ...
  const renderCategories = () => (
    <div className="grid lg:grid-cols-[1fr_360px] gap-6">
      <div className="space-y-3">
        {rows.categories?.length ? rows.categories.filter((category: any) => matches(category.name, category.slug, category.description)).map((category: any) => <div key={category.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">{category.name}</p><p className="text-[11px] font-mono text-kurla-amber">{category.slug}</p></div><button className={mutedButtonClass} onClick={() => { setEditingCategory(category); setCategoryForm({ name: category.name || '', slug: category.slug || '', description: category.description || '' }); }}><Pencil className="w-3.5 h-3.5" /> Modifier</button></div>) : renderEmpty('Aucune catégorie métier enregistrée.')}
      </div>
      <form className="p-5 rounded-2xl bg-kurla-ink border border-kurla-cream/10 space-y-3" onSubmit={e => { e.preventDefault(); saveEntity('category', categoryForm, editingCategory?.id); }}>
        <h3 className="text-sm font-semibold flex items-center gap-2"><FolderTree className="w-4 h-4 text-kurla-amber" /> {editingCategory ? 'Modifier la catégorie' : 'Ajouter une catégorie'}</h3>
  ...
  const renderPayments = () => (
    <div className="space-y-3">{rows.payments?.length ? rows.payments.filter((payment: any) => matches(payment.id, payment.order_id, payment.status, payment.stripe_payment_intent_id)).map((payment: any) => <div key={payment.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 grid md:grid-cols-[1fr_auto_auto] items-center gap-3"><div><p className="text-sm font-semibold">{Number(payment.amount || 0).toFixed(2)} {payment.currency || 'EUR'}</p><p className="text-[11px] text-kurla-cream/45 font-mono">Commande {payment.order_id || 'non renseignée'} · {payment.stripe_payment_intent_id || 'sans identifiant Stripe'}</p></div><span className="text-[11px] text-kurla-amber">{payment.status}</span><select className={inputClass + ' md:w-44'} value={payment.status} onChange={e => run(`payment-${payment.id}`, { url: `/api/admin/payments/${payment.id}/status`, method: 'POST', body: JSON.stringify({ status: e.target.value }) })}><option value="pending">pending</option><option value="succeeded">succeeded</option><option value="failed">failed</option><option value="partially_refunded">partially_refunded</option><option value="refunded">refunded</option></select></div>) : renderEmpty('Aucun paiement persistant.')}</div>
  );
  ...
  const renderShipments = () => (
    <div className="space-y-3">{shipments.length ? shipments.filter((shipment: any) => matches(shipment.id, shipment.orderId, shipment.order_id, shipment.status, shipment.tracking_number, shipment.trackingNumber, shipment.country)).map((shipment: any) => { const draft = shipmentValue(shipment); const address = shipment.address || shipment.delivery_address; return <div key={shipment.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 space-y-3">
      <div className="flex flex-wrap justify-between gap-2"><div><p className="text-sm font-semibold">Commande <span className="font-mono">{shipment.orderId || shipment.order_id}</span></p><p className="text-[11px] text-kurla-cream/50">{address ? `${address.fullName}, ${address.street}, ${address.postalCode} ${address.city} · ${address.country}` : 'Adresse de livraison non renseignée.'}</p></div><span className="text-[11px] text-kurla-amber">{shipment.status}</span></div>
      ...
      {shipmentHistories[shipment.orderId || shipment.order_id] && <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10 space-y-1"><p className="text-[10px] uppercase text-kurla-cream/45 font-bold">Historique des événements</p>{shipmentHistories[shipment.orderId || shipment.order_id].map((event: any) => <p key={event.id} className="text-[11px] text-kurla-cream/65"><span className="font-mono text-kurla-amber">{new Date(event.createdAt).toLocaleString('fr-FR')}</span> · {event.status}{event.location ? ` · ${event.location}` : ''}{event.description ? ` — ${event.description}` : ''}</p>)}</div>}<div className="flex flex-wrap gap-2"><button type="button" className={mutedButtonClass} onClick={() => loadShipmentHistory(shipment.orderId || shipment.order_id)}>Voir l’historique</button><p className="text-[11px] text-kurla-cream/45 self-center">Le statut expédié/en transit/livré exige le numéro réel saisi par l’opérateur. Aucun suivi n’est généré automatiquement.</p></div><button className={buttonClass} onClick={() => run(`shipment-${shipment.id}`, { url: `/api/admin/shipments/${shipment.orderId || shipment.order_id}`, method: 'PATCH', body: JSON.stringify(draft) })}><Truck className="w-3.5 h-3.5" /> Enregistrer et journaliser</button></div>; }) : renderEmpty('Aucune expédition enregistrée. Les expéditions sont créées depuis une commande.')}</div>
  );

  const renderUsers = () => (
    <div className="space-y-3">{users.length ? users.filter((user: any) => matches(user.email, user.full_name, user.role, user.id)).map((user: any) => <div key={user.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 grid md:grid-cols-[1fr_auto] items-center gap-3"><div><p className="text-sm font-semibold">{user.full_name || 'Nom non renseigné'}</p><p className="text-[11px] text-kurla-cream/50">{user.email} · créé le {user.created_at ? new Date(user.created_at).toLocaleDateString('fr-FR') : 'date absente'}</p></div><div className="flex items-center gap-2"><span className="text-[10px] text-kurla-cream/45">{user.role}</span><select className={inputClass + ' w-40'} value={user.role} onChange={e => run(`user-${user.id}`, { url: `/api/admin/users/${user.id}/role`, method: 'POST', body: JSON.stringify({ role: e.target.value }) }, 'Rôle utilisateur mis à jour.') }><option value="customer">customer</option><option value="professional">professional</option><option value="support">support</option><option value="editor">editor</option><option value="admin">admin</option><option value="superadmin">superadmin</option></select></div></div>) : renderEmpty('Aucun profil utilisateur chargé.')}</div>
  );

  const renderArticles = () => (
    <div className="grid xl:grid-cols-[1fr_500px] gap-6">
      <div className="space-y-3">
        {rows.articles?.length ? rows.articles.filter((article: any) => matches(article.title, article.slug, article.status, article.category, article.contentType, article.topic)).map((article: any) => <div key={article.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 flex justify-between gap-3"><div><p className="text-sm font-semibold">{article.title}</p><p className="text-[11px] text-kurla-amber">/{article.slug} · {article.contentType || 'article'} · {article.status}</p><p className="text-[11px] text-kurla-cream/45">{article.topic || 'thématique non renseignée'} · auteur : {article.author || 'non renseigné'} · preuve : {article.evidenceLevel || 'non renseignée'}</p></div><button className={mutedButtonClass} onClick={() => setArticleForm(articleFormFromRow(article))}><Pencil className="w-3.5 h-3.5" /> Modifier</button></div>) : renderEmpty('Aucun contenu éditorial persistant. Créez un brouillon pour commencer.')}
      </div>
      <form className="p-5 rounded-2xl bg-kurla-ink border border-kurla-cream/10 space-y-2.5 max-h-[calc(100vh-220px)] overflow-y-auto" onSubmit={e => { e.preventDefault(); saveEntity('content', { ...articleForm, relatedProductIds: typeof articleForm.relatedProductIds === 'string' ? articleForm.relatedProductIds.split(',').map((id: string) => id.trim()).filter(Boolean) : (articleForm.relatedProductIds || []) }, articleForm.id); }}>
        <h3 className="text-sm font-semibold">{articleForm.id ? 'Modifier un contenu éditorial' : 'Nouveau contenu éditorial'}</h3>
        <p className="text-[11px] text-kurla-cream/45">Les champs manquants restent explicitement non renseignés. Une publication est bloquée sans auteur, source, niveau de preuve, langue et traduction.</p>
  ...
  const renderRetention = () => (
    <div className="space-y-4">
      ...
      {retentionResult && (
        <div className="p-5 rounded-2xl bg-kurla-ink border border-kurla-amber/30 space-y-3">
          <p className="text-sm font-semibold">Dernier calcul</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="p-3 rounded-xl bg-kurla-espresso">
              <p className="text-2xl font-serif-title font-bold text-kurla-amber">{retentionResult.usersScanned ?? 0}</p>
              <p className="text-[11px] text-kurla-cream/55">utilisateurs analysés</p>
            </div>
            <div className="p-3 rounded-xl bg-kurla-espresso">
              <p className="text-2xl font-serif-title font-bold text-emerald-300">{retentionResult.nudgesCreated ?? 0}</p>
              <p className="text-[11px] text-kurla-cream/55">relances créées</p>
            </div>
            <div className="p-3 rounded-xl bg-kurla-espresso col-span-2 sm:col-span-1">
              <p className="text-[11px] text-kurla-cream/55 mb-1">Par type</p>
              <p className="text-[11px] text-kurla-cream/75">
      ...
      <div className="p-5 rounded-2xl bg-kurla-espresso border border-kurla-cream/10 space-y-3">
        <p className="text-sm font-semibold flex items-center gap-2">
          <ShoppingBag className="w-4 h-4 text-kurla-amber" /> Récupération des paniers abandonnés
        </p>
        <p className="text-[11px] text-kurla-cream/55 leading-relaxed">
        ...
        {abandonedResult && (
          <div className="p-4 rounded-xl bg-kurla-ink border border-kurla-amber/30">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
              <div><p className="text-xl font-bold text-kurla-cream">{abandonedResult.eligible ?? 0}</p><p className="text-[10px] text-kurla-cream/55">à relancer</p></div>
              <div><p className="text-xl font-bold text-emerald-300">{abandonedResult.emailsSent ?? 0}</p><p className="text-[10px] text-kurla-cream/55">emails envoyés</p></div>
              <div><p className="text-xl font-bold text-kurla-amber">{abandonedResult.emailsLogged ?? 0}</p><p className="text-[10px] text-kurla-cream/55">journalisés (mode test)</p></div>
              <div><p className="text-xl font-bold text-rose-300">{abandonedResult.emailsFailed ?? 0}</p><p className="text-[10px] text-kurla-cream/55">échecs</p></div>
            </div>
  ...
  const renderLogs = () => (
    <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr className="border-b border-kurla-cream/10 text-kurla-amber"><th className="py-3 pr-4">Date</th><th className="py-3 pr-4">Action</th><th className="py-3 pr-4">Administrateur</th><th className="py-3">Détails</th></tr></thead><tbody className="divide-y divide-kurla-cream/5">{rows.logs?.length ? rows.logs.filter((log: any) => matches(log.action, log.user_id, log.userId, JSON.stringify(log.details))).map((log: any) => <tr key={log.id}><td className="py-3 pr-4 whitespace-nowrap">{log.created_at ? new Date(log.created_at).toLocaleString('fr-FR') : log.createdAt}</td><td className="py-3 pr-4 font-mono text-kurla-amber">{log.action}</td><td className="py-3 pr-4 font-mono text-kurla-cream/55">{log.user_id || log.userId || 'système'}</td><td className="py-3 max-w-md truncate text-kurla-cream/50">{JSON.stringify(log.details || {})}</td></tr>) : <tr><td colSpan={4} className="py-8 text-center text-kurla-cream/45">Aucun log d’administration persistant.</td></tr>}</tbody></table></div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap gap-2">
        {nav.map(([id, label, Icon]) => <button key={id} onClick={() => setSection(id)} className={`px-3 py-2 rounded-xl text-[11px] font-semibold flex items-center gap-2 ${section === id ? 'bg-kurla-copper text-white' : 'bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/65 hover:text-white'}`}><Icon className="w-3.5 h-3.5" />{label}</button>)}
      </div>
      ...
      <div className="p-4 rounded-2xl bg-kurla-espresso border border-kurla-cream/10 flex flex-wrap items-center gap-3">
        <RefreshCw className={`w-5 h-5 text-kurla-copper ${reconciling ? 'animate-spin' : ''}`} />
        <div className="flex-1 min-w-[16rem]">
          <p className="text-sm font-semibold text-kurla-cream">Vérifier les paiements auprès de Stripe</p>
          <p className="text-xs text-kurla-cream/55 leading-relaxed">
          ...
          {reconcileResult && <p className="text-xs text-kurla-amber mt-1.5">{reconcileResult}</p>}
        </div>
        <button type="button" onClick={runReconciliation} disabled={reconciling}
          className="px-4 py-2 rounded-xl bg-kurla-copper hover:bg-[#D98A4E] disabled:opacity-50 text-white text-xs font-semibold transition-colors">
      ...
      <div className="p-6 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-5 shadow-xl">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-serif-title font-bold flex items-center gap-2"><Layers3 className="w-5 h-5 text-kurla-copper" />{sectionTitle}</h2><p className="text-xs text-kurla-cream/50 mt-1">Source : Supabase. Les mutations sensibles sont réservées aux rôles admin et journalisées.</p></div><div className="flex items-center gap-2"><input className={inputClass + ' sm:w-64'} placeholder="Filtrer cette section…" value={filter} onChange={e => setFilter(e.target.value)} />{message && <span className="text-xs text-kurla-amber max-w-sm">{message}</span>}</div></div>
        {content}
      </div>
    </div>
  );
