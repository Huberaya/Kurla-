...
const MOMENT_META: Record<RoutineMoment, { label: string; icon: React.ReactNode; tone: string }> = {
  matin: { label: 'Matin · protéger', icon: <Sun className="w-5 h-5 text-[#C8753D]" />, tone: 'bg-[#FFF7EF]' },
    ...
    <div className="min-h-screen pt-28 pb-24 bg-[#FFFDF9] text-[#111111]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <header className="max-w-3xl mx-auto text-center">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#111111] text-[#D9A8A4] text-[10px] font-bold tracking-widest uppercase">KURLA SKIN · routine adaptative V1</span>
          <h1 className="text-3xl sm:text-5xl font-serif-title font-bold mt-3">Une routine construite à partir de votre profil</h1>
          <p className="text-sm text-[#111111]/65 font-light leading-relaxed mt-3">
        ...
        <section className="p-5 rounded-3xl bg-[#F8F2EC] border border-[#E8E1DA]">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-widest font-bold text-[#C8753D]">Niveau de routine</p>
              <p className="text-xs text-[#111111]/60 mt-1">Le niveau change le nombre d’étapes, pas les règles de sécurité.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {TIERS.map(option => (
                <button
                  ...
                  className={`px-4 py-2.5 rounded-full border text-xs font-bold ${tier === option.value ? 'bg-[#111111] text-white border-[#111111]' : 'bg-white border-[#E8E1DA] hover:border-[#C8753D]'}`}
                >
                  {option.label}
                </button>
              ))}
          ...
          <p className="text-xs text-[#111111]/60 mt-3">{TIERS.find(option => option.value === tier)?.hint}</p>
        </section>

        <section className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="p-4 rounded-2xl bg-white border border-[#E8E1DA]"><p className="text-[10px] uppercase tracking-wider text-[#C8753D] font-bold">Profil</p><p className="text-sm font-bold mt-1">{skin.skinType === 'inconnu' ? 'Partiel' : skin.skinType}</p><p className="text-[11px] text-[#111111]/55 mt-1">Type de peau déclaré</p></div>
          <div className="p-4 rounded-2xl bg-white border border-[#E8E1DA]"><p className="text-[10px] uppercase tracking-wider text-[#C8753D] font-bold">Phototype</p><p className="text-sm font-bold mt-1">{phototypeKnown ? `Type ${skin.phototype}` : 'Non renseigné'}</p><p className="text-[11px] text-[#111111]/55 mt-1">Aucune déduction depuis toneDepth</p></div>
          <div className="p-4 rounded-2xl bg-white border border-[#E8E1DA]"><p className="text-[10px] uppercase tracking-wider text-[#C8753D] font-bold">Catalogue</p><p className="text-sm font-bold mt-1">{knownProducts.length} produit{knownProducts.length > 1 ? 's' : ''} sélectionné{knownProducts.length > 1 ? 's' : ''}</p><p className="text-[11px] text-[#111111]/55 mt-1">{products.length} produit{products.length > 1 ? 's' : ''} reçu{products.length > 1 ? 's' : ''} du serveur</p></div>
        </section>
        ...
        {profileLoading && <p className="text-xs text-[#111111]/50 text-center">Synchronisation du profil enregistré…</p>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          {(['matin', 'soir', 'hebdo'] as RoutineMoment[]).map(moment => {
            ...
            return (
              <section key={moment} className={`p-5 rounded-3xl border border-[#E8E1DA] ${meta.tone}`}>
                <h2 className="text-base font-bold flex items-center gap-2">{meta.icon}{meta.label}</h2>
                <ol className="mt-4 space-y-3">
                  {steps.map(step => (
                    <li key={`${step.moment}-${step.order}`} className="p-3 rounded-2xl bg-white/80 border border-[#E8E1DA]">
                      <p className="text-[10px] uppercase tracking-wider text-[#C8753D] font-bold">{step.order}. {step.label}</p>
                      {step.product ? (
                        <>
                          <p className="text-sm font-bold mt-1">{step.product.name}</p>
                          <p className="text-[11px] text-[#111111]/55 mt-1">{step.why}</p>
                          <div className="flex items-center justify-between gap-2 mt-2"><span className="text-xs font-bold">{Number(step.product.price).toFixed(2)} €</span><a href={`/produit/${step.product.slug}`} className="text-[11px] px-2.5 py-1 rounded-full bg-[#C8753D] text-white font-bold">Voir la fiche</a></div>
                        </>
                      ) : (
                        <p className="text-xs text-[#111111]/55 mt-1">Aucune fiche publiée et sélectionnable pour cette étape. KURLA n’invente pas de produit.</p>
                      )}
        ...
        <section className="p-5 rounded-3xl bg-[#F8F2EC] border border-[#E8E1DA]">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div><p className="text-[10px] uppercase tracking-widest text-[#C8753D] font-bold flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> Observance</p><p className="text-sm font-bold mt-1">Cochez les étapes réalisées, sans suivi automatique.</p><p className="text-[11px] text-[#111111]/55 mt-1">Streak matin : {streaks.matin}j · soir : {streaks.soir}j</p></div>
            <div className="flex gap-2">
              <button type="button" onClick={() => markDone('matin')} className={`px-4 py-2 rounded-full border text-xs font-bold ${observance.matin ? 'bg-[#C8753D] text-white border-[#C8753D]' : 'bg-white border-[#E8E1DA]'}`}>Matin {observance.matin ? '✓' : ''}</button>
              <button type="button" onClick={() => markDone('soir')} className={`px-4 py-2 rounded-full border text-xs font-bold ${observance.soir ? 'bg-[#111111] text-white border-[#111111]' : 'bg-white border-[#E8E1DA]'}`}>Soir {observance.soir ? '✓' : ''}</button>
            </div>
          </div>
        </section>

        <section className="p-5 rounded-3xl bg-[#111111] text-white flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div><p className="text-[10px] uppercase tracking-widest text-[#D49A63] font-bold">Prix réel de la sélection</p><p className="text-xl font-bold mt-1">{routine.totalPrice.toFixed(2)} €</p><p className="text-[11px] text-white/55 mt-1">Somme des prix reçus du catalogue serveur ; aucun prix inventé.</p></div>
          <div className="text-xs text-white/70 flex gap-2 items-start max-w-md"><Info className="w-4 h-4 shrink-0 text-[#D49A63]" /><span>{unknownSteps.length ? `${unknownSteps.length} étape${unknownSteps.length > 1 ? 's' : ''} reste${unknownSteps.length > 1 ? 'nt' : ''} sans fiche sélectionnable. Les kits et références de formulation cible ne sont pas rendus achetables.` : 'Toutes les étapes disposent d’une fiche sélectionnable.'}</span></div>
        </section>

        <div className="flex flex-wrap gap-2 justify-center text-xs">
          <a href="/peau/diagnostic" className="px-4 py-2 rounded-full bg-white border border-[#E8E1DA] font-bold hover:border-[#C8753D]">Modifier mon profil</a>
          <a href="/boutique?cat=peau" className="px-4 py-2 rounded-full bg-[#C8753D] text-white font-bold">Explorer le catalogue peau</a>
        </div>
      </div>
    </div>
  );
