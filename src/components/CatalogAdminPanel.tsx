...
import { badgesForDropshipToggle, canPersistDropship24hBadge, isSkinCosmeticCategory, promisesDropship24h } from '../lib/dropshipProcedure';
...
type CatalogAdminPanelProps = {
  ...
  focusLabel?: string;
  /** Renvoie vers la base fournisseurs de l'Approvisionnement (17/09). La
   *  modification d'un fournisseur ne se fait PAS ici : cette fiche produit
   *  n'est qu'un point de lecture qui mène à l'unique surface d'édition. */
...
export const CatalogAdminPanel: React.FC<CatalogAdminPanelProps> = ({ headers, onSuccess, onOpenGuide, scope = 'all', focusProductId, focusLabel, onOpenSupplierBase }) => {
  ...
  const columnFilters = useMemo<ColumnFilter[]>(() => [
    { key: 'name', ...listFilter({ key: 'name', rows: products, get: (product: any) => product.name, extra: (product: any) => [product.slug] }) },
    // 17/09, demande « listes déroulantes sur tous les tableaux » : les marques
    // sont un ensemble fermé, on les propose au lieu de les faire taper.
    { key: 'brand', ...listFilter({ key: 'brand', rows: products, get: (product: any) => product.brand, emptyLabel: 'Marque non renseignée' }) },
  ...
  ], [categories, suppliersRef, products]);
  ...
  /** Densité (17/09) : une fiche produit condensée = le nom, les statuts, le
   *  prix, le stock et le nombre de manques. Les blocs secondaires (détail des
   *  manques, fournisseur, badges de score) passent derrière « Tout afficher » :
   *  rien n'est supprimé, seulement replié. */
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <form onSubmit={saveDraft} className="p-6 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <label className={`col-span-2 flex items-start gap-3 p-3 rounded-xl border ${canPersistDropship24hBadge(scope === 'skin' ? 'peau' : draft.category) ? 'bg-emerald-500/10 border-emerald-500/20 cursor-pointer' : 'bg-rose-950/20 border-rose-500/20'}`}>
              <input type="checkbox" checked={draft.isDropship && canPersistDropship24hBadge(scope === 'skin' ? 'peau' : draft.category)} onChange={e => setField('isDropship', e.target.checked)} disabled={!canPersistDropship24hBadge(scope === 'skin' ? 'peau' : draft.category)} className="mt-0.5 accent-emerald-500" />
              <span className="text-[11px] leading-tight">
                <span className="font-bold text-emerald-300">Dropship 24–48h (partenaire UE) — 0 stock chez toi</span>
                {isSkinCosmeticCategory(scope === 'skin' ? 'peau' : draft.category)
                  ? <span className="block text-rose-200/80">Cosmétique Skin ≠ dropship 24–48h (CPNP / douane). L’offre se saisit dans <code className="font-mono">product_sources</code> (affiliation ou 3PL). La case ne persiste aucun badge.</span>
