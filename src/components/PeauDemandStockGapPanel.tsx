  ...
  return (
    <div className="rounded-3xl bg-[#1A0F0A] border border-[#FFF7EF]/10 p-6 sm:p-8 space-y-6 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-lg font-serif-title font-bold text-[#FFF7EF] flex items-center gap-2">
            <Gauge className="w-5 h-5 text-[#C8753D]" /> C18 — Cockpit peau : demande vs stock → reste à sourcer
          ...
          <p className="text-xs text-[#FFF7EF]/60 mt-1 max-w-3xl leading-relaxed">
            <strong className="text-[#FFF7EF]">Demande peau</strong> = commandes réglées + en attente + unités induites par kits (1 kit = 1× chaque composant) — <strong className="text-[#FFF7EF]">Stock</strong> = lots reçus · <strong className="text-emerald-300">Reste à sourcer</strong> = demande − stock (jamais négatif) · commande fournisseur = max(MOQ 50, gap). Rien n’est estimé : sans lot, le stock vaut 0.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="p-3 rounded-2xl bg-[#050403] border border-emerald-500/25 text-center">
          <p className="text-[10px] uppercase tracking-wider font-bold text-emerald-300">Kits fermes</p>
          <p className="text-xl font-bold text-[#FFF7EF]">{totals.kitsFirm}</p>
          <p className="text-[10px] text-[#FFF7EF]/40">KPEAU-01/02/03</p>
        </div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-amber-500/25 text-center">
          <p className="text-[10px] uppercase tracking-wider font-bold text-amber-300">Kits en attente</p>
          <p className="text-xl font-bold text-[#FFF7EF]">{totals.kitsPending}</p>
          <p className="text-[10px] text-[#FFF7EF]/40">Stripe TEST</p>
        </div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-sky-500/25 text-center">
          <p className="text-[10px] uppercase tracking-wider font-bold text-sky-300">Unités peau à sourcer</p>
          <p className="text-xl font-bold text-[#FFF7EF]">{totals.qtyToSource}</p>
          <p className="text-[10px] text-[#FFF7EF]/40">7 SKU peau (hors kits)</p>
        </div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-[#FFF7EF]/10 text-center">
          <p className="text-[10px] uppercase tracking-wider font-bold text-[#D49A63]">Reçues (lots)</p>
          <p className="text-xl font-bold text-[#FFF7EF]">{totals.received}</p>
          <p className="text-[10px] text-[#FFF7EF]/40">{coverage}% couverture</p>
        </div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-rose-500/25 text-center">
          ...
          <p className="text-[10px] text-[#FFF7EF]/40">sans MOQ</p>
        </div>
        <div className="p-3 rounded-2xl bg-[#050403] border border-[#C8753D]/30 text-center">
          <p className="text-[10px] uppercase tracking-wider font-bold text-[#C8753D]">À commander (MOQ 50)</p>
          <p className="text-xl font-bold text-[#C8753D]">{totals.toOrder}</p>
          <p className="text-[10px] text-[#FFF7EF]/40">par SKU ≥50</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[11px] text-[#FFF7EF]/45 hidden sm:inline">7 composants peau — kit = 1× chaque composant</span>
      </div>
        ...
        <table className="w-full text-left text-xs border-collapse">
          ...
          <tbody className="divide-y divide-[#FFF7EF]/5">
            {PEAU_KITS.map(k=>{
              ...
              return (
                <tr key={k.sku}>
                  <td className="px-3 py-2"><span className="font-bold text-[#FFF7EF]">{k.id} {k.tier}</span><span className="text-[#FFF7EF]/40"> · {k.sku}</span></td>
                  ...
                  <td className="px-3 py-2 text-right font-mono text-[#D49A63]">{k.priceBundle.toFixed(2)}€</td>
                </tr>
              );
            })}
          </tbody>
            ...
            {comps.map(c=> (
              <tr key={c.id} className={c.gap>0?'bg-rose-950/10':''}>
                <td className="px-3 py-2">
                  <span className="font-semibold text-[#FFF7EF]">{c.name}</span>
                  <span className="text-[10px] text-[#FFF7EF]/40 ml-1.5 font-mono">{c.id}</span>
                  <span className="text-[10px] text-[#FFF7EF]/50 block">{c.role}</span>
                </td>
                <td className="px-2 py-2 text-center">
                  <span className="font-bold text-[#FFF7EF]">{c.qtyToSource}</span>
                  <span className="text-[10px] text-[#FFF7EF]/40 block">F{c.qtyFirm} + P{c.qtyPending} + K{c.qtyFromKits}</span>
                </td>
                <td className="px-2 py-2 text-center"><span className={c.received? 'text-emerald-300 font-bold':'text-[#FFF7EF]/40'}>{c.received}</span></td>
                <td className="px-2 py-2 text-center">
                ...
                <td className="px-2 py-2 text-center">
                  {c.toOrder>0 ? <span className="px-2 py-1 rounded-full bg-[#C8753D]/20 border border-[#C8753D]/40 text-[#F3C9A4] font-bold">{c.toOrder}</span>
                  : <span className="text-[#FFF7EF]/30">—</span>}
                </td>
                <td className="px-3 py-2 text-[11px] leading-snug">
                  ...
                  : <span className="text-[#FFF7EF]/60">{c.qtyFromKits>0?`Dont ${c.qtyFromKits} via kits. `:''} Commander {c.toOrder}.</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex flex-wrap gap-2 text-[11px]">
        ...
        <span className="px-3 py-1.5 rounded-full bg-[#050403] border border-[#FFF7EF]/10 text-[#FFF7EF]/60 flex items-center gap-1.5"><Boxes className="w-3.5 h-3.5" /> Lots reçus = réel, pas cible</span>
        ...
        <a href="#batches" onClick={e=>{e.preventDefault(); document.querySelector('[data-tab=batches]')?.scrollIntoView();}} className="px-3 py-1.5 rounded-full bg-[#C8753D] text-white font-bold flex items-center gap-1">Voir Lots → <ArrowRight className="w-3 h-3" /></a>
      </div>

      <p className="text-[10px] text-[#FFF7EF]/35 text-center leading-relaxed">
        C18 — sources : <span className="text-[#D49A63]">/api/admin/preorder-demand</span> (qtyFirm/qtyPending/qtyFromKits/qtyToSource) + <span className="text-[#D49A63]">/api/admin/batches</span> (quantityReceived) + <span className="text-[#D49A63]">peauKits.ts</span> (7 composants, 3 kits). Gap = max(0, demande − reçu) · À commander = max(50, gap). Aucune estimation affichée comme stock.
      </p>
    </div>
  );
};
