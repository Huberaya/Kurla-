    ...
    <div className="rounded-3xl bg-kurla-espresso border border-kurla-cream/10 p-6 sm:p-8 space-y-7 shadow-xl">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h3 className="text-lg font-serif-title font-bold text-kurla-cream flex items-center gap-2">
            <Receipt className="w-5 h-5 text-kurla-copper" /> C23 — Facturation Stripe TEST→LIVE FR82/BE76 + suivi livraison
          ...
          <p className="text-xs text-kurla-cream/60 mt-1 max-w-3xl leading-relaxed">
            Passage LIVE <strong className="text-kurla-cream">pays par pays (FR 82/BE 76)</strong> — pas un switch global. LIVE exige <strong className="text-emerald-300">sk_live + 8 gates verts fichier+date + 1 lot 50 reçu</strong>. Tant que TEST, tout encaissement = simulation (aucun euro réel), mais <strong className="text-kurla-cream">la facture reste reconstituable</strong> (snapshot TVA stocké). Livraison : seul un <strong className="text-kurla-cream">numéro réel transporteur</strong> fait passer <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">shipped</code>.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className={`p-3 rounded-2xl border text-center ${stripeMode==='live'?'bg-emerald-500/10 border-emerald-500/20':'bg-amber-500/10 border-amber-500/20'}`}>
          ...
          <p className="text-[10px] text-kurla-cream/40 mt-1">{stripeMode==='live'?'sk_live détectée':'sk_test / absente'}</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-lg font-bold text-kurla-cream flex items-center justify-center gap-1.5"><Euro className="w-4 h-4" /> {paidOrders.length}</p>
          <p className="text-[10px] uppercase tracking-wider text-kurla-amber">Commandes facturables (paid+)</p>
          <p className="text-[10px] text-kurla-cream/40">{pendingOrders.length} en attente webhook</p>
        </div>
        <div className="p-3 rounded-2xl bg-kurla-ink border border-kurla-cream/10 text-center">
          <p className="text-lg font-bold text-kurla-cream">{metrics?.avgOrderValue ? `${Number(metrics.avgOrderValue).toFixed(2)}€` : '—'}</p>
          <p className="text-[10px] uppercase tracking-wider text-kurla-amber">AOV (paid)</p>
          <p className="text-[10px] text-kurla-cream/40">net+ TVA = TTC encaissé</p>
        </div>
        <div className="p-3 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 text-center">
          ...
          <p className="text-[10px] text-kurla-cream/40">préparing→delivered</p>
        </div>
      </div>

      {/* Bornes FR82/BE76 */}
      <div className="flex flex-wrap gap-1.5 text-[10px]">
        <span className={`px-2.5 py-1 rounded-full border font-bold flex items-center gap-1 ${effective('FR')==='live'?'bg-emerald-500/15 text-emerald-300 border-emerald-500/30':'bg-kurla-ink border-kurla-cream/10 text-kurla-cream/60'}`}><Globe className="w-3 h-3" /> FR 82 {effective('FR')==='live'?'LIVE':'TEST'} · {frCfg.dispatchDays}</span>
        ...
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> 8 gates fichier+date requis pour LIVE</span>
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><Clock className="w-3 h-3" /> Préco lun/jeu 18h · 0 stock Paris</span>
        <span className="px-2.5 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-kurla-cream/60 flex items-center gap-1"><Receipt className="w-3 h-3" /> EUR seule · OSS · TVA destination</span>
      </div>
              ...
              return (
                <tr key={c.code} className={isLive ? 'bg-emerald-950/10' : c.code==='FR' ? 'bg-amber-950/10' : ''}>
                  <td className="py-2.5 px-3">
                    <span className="font-bold text-kurla-cream flex items-center gap-1.5">{c.flag} {c.code} · {c.name} <span className={`ml-1 px-1.5 py-0.5 rounded-full border text-[10px] font-bold ${c.score>=70?'bg-kurla-copper/15 text-kurla-amber border-kurla-copper/30':'bg-kurla-cream/5 text-kurla-cream/50 border-kurla-cream/10'}`}>{c.score}/100</span></span>
                    <span className="text-[10px] text-kurla-cream/40">{c.hub} · {c.currency}</span>
                  </td>
                  <td className="py-2.5 px-3">
                    <span className={`px-2 py-1 rounded-full border text-[10px] font-bold ${isLive?'bg-emerald-500/15 text-emerald-300 border-emerald-500/30':'bg-kurla-cream/5 text-kurla-cream/50 border-kurla-cream/10'}`}>{eff.toUpperCase()}</span>
                    {c.code==='FR' && <span className="block text-[10px] text-kurla-cream/40 mt-1">{stripeMode==='live'?'sk_live → LIVE si gates verts':'TEST (sk_test/aucune)'} </span>}
                    {c.code==='BE' && <span className="block text-[10px] text-kurla-cream/40 mt-1">J+30 si FR {'>'}30% J+7</span>}
                  </td>
                  <td className="py-2.5 px-3 text-kurla-cream/80 leading-snug">{c.dispatch}<span className="block text-[10px] text-kurla-cream/40">{c.dispatchDays}</span></td>
                  <td className="py-2.5 px-3 font-mono text-kurla-cream/80">{c.vatRate}%</td>
                  <td className="py-2.5 px-3 text-kurla-cream/70 leading-snug">{c.model}</td>
                  <td className="py-2.5 px-3 text-kurla-cream/60 leading-snug text-[11px]">{c.condition}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <p className="text-[11px] text-kurla-cream/35 leading-relaxed">Source scoring : <span className="text-kurla-amber">docs/sourcing/pays_scoring.md (FR 82/BE 76/SN 71/CI 68/MA 64/CH 58/CM 52)</span> + <span className="text-kurla-amber">src/lib/countryFulfillment.ts</span>. Règle : <strong className="text-kurla-cream">score ≥65 + fournisseur MOQ &lt;100 + whitecast V-VI faible</strong> pour envisager l'ouverture ; sinon TEST bloqué — voulu.</p>

      {/* Facturation */}
      <div className="p-4 rounded-2xl bg-kurla-ink border border-kurla-copper/30 space-y-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <h4 className="text-sm font-bold text-kurla-cream flex items-center gap-2"><FileText className="w-4 h-4 text-kurla-copper" /> Facturation : TTC encaissé = snapshot reconstituable</h4>
          <span className="px-2 py-1 rounded-full bg-kurla-ink border border-kurla-cream/10 text-[10px] text-kurla-cream/60">EUR seule · price_includes_vat · TVA destination (art.33)</span>
        </div>

        <div className="grid sm:grid-cols-3 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="font-bold text-kurla-cream flex items-center gap-1.5"><Euro className="w-3.5 h-3.5" /> TTC encaissé</p>
            <p className="text-kurla-cream/60 leading-relaxed mt-1">Prix catalogue TTC (ou HT → TTC si HT). La TVA est <em>extraite</em> au taux du pays de livraison, pas ajoutée après. <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10 text-[10px]">vatBreakdown[]</code> stocké ligne par ligne.</p>
          </div>
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="font-bold text-kurla-cream flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5" /> Auto-liquidation</p>
            <p className="text-kurla-cream/60 leading-relaxed mt-1">Assujetti UE avec <strong className="text-kurla-cream">VIES vérifié</strong> → net facturé, TVA 0 (art.138/196). Sinon TVA normale. Vérif via <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">verifyVatNumber</code> (log <code>vatCheckedAt</code>).</p>
          </div>
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="font-bold text-kurla-cream flex items-center gap-1.5"><Receipt className="w-3.5 h-3.5" /> Preuve comptable</p>
            <p className="text-kurla-cream/60 leading-relaxed mt-1">Chaque commande porte <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">currency/vatCountry/netAmount/vatAmount/breakdown</code> + <code>shippingAmountCents</code>. Reconstituable sans Stripe — coffre OSS.</p>
          </div>
        </div>
              ...
              <table className="w-full text-left text-xs border-collapse min-w-[760px]">
                ...
                <tbody className="divide-y divide-kurla-cream/5">
                  {last5Paid.map(o=>{
                    ...
                    return (
                      <tr key={o.id} className="hover:bg-kurla-espresso/60">
                        <td className="py-2.5 px-3"><span className="font-mono font-bold text-kurla-cream">{o.id}</span><span className="block text-[10px] text-kurla-cream/40">{maskEmail(o.customerEmail)}</span></td>
                        <td className="py-2.5 px-3"><span className="text-kurla-cream/80">{new Date(o.createdAt).toLocaleDateString('fr-FR')}</span><span className={`ml-1 px-1.5 py-0.5 rounded-full border text-[10px] font-bold ${['paid','delivered'].includes(o.status)?'bg-emerald-500/15 text-emerald-300 border-emerald-500/30':['processing','packed','shipped'].includes(o.status)?'bg-sky-500/15 text-sky-300 border-sky-500/30':'bg-amber-500/15 text-amber-300 border-amber-500/30'}`}>{o.status}</span></td>
                        <td className="py-2.5 px-3"><span className="text-kurla-cream/80">{countryLabel(country)}</span><span className="block text-[10px] text-kurla-cream/40">{o.currency || 'EUR'} · {o.customerVatNumber ? `VIES ${o.customerVatNumber}` : 'particulier'}</span></td>
                        <td className="py-2.5 px-3 font-mono text-kurla-cream/80">{net !== '—' ? `${net}€` : '—'}</td>
                        <td className="py-2.5 px-3 font-mono text-kurla-cream/80">{vat !== '—' ? `${vat}€` : '—'}<span className="block text-[10px] text-kurla-cream/40">{vatRate}</span></td>
                        <td className="py-2.5 px-3 font-mono font-bold text-kurla-cream">{Number(o.total).toFixed(2)}€</td>
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={()=>copy(factureMentions,'facture')} className="px-3 py-1.5 rounded-full bg-kurla-copper hover:bg-kurla-amber text-white text-xs font-bold flex items-center gap-1.5">
            {copyOk==='facture' ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />} {copyOk==='facture'?'Copié':'Copier mentions facture (OSS)'}
          </button>
          <a href={stripeMode==='live'? 'https://dashboard.stripe.com/payments' : 'https://dashboard.stripe.com/test/payments'} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-full bg-kurla-ink border border-kurla-cream/15 text-xs text-kurla-cream/70 flex items-center gap-1.5 hover:border-kurla-copper">
            <ExternalLink className="w-3.5 h-3.5" /> Ouvrir Stripe {stripeMode==='live'?'LIVE':'TEST'} <span className="text-[10px] text-kurla-cream/40">dashboard</span>
          </a>
          <span className="text-[11px] text-kurla-cream/35 self-center">En TEST, « émise (snapshot) » = preuves TVA sans flux bancaire — la compta voit déjà la ventilation exacte.</span>
        </div>
      </div>

      {/* IBAN / payout */}
      <div className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/10 space-y-3">
        <h4 className="text-sm font-bold text-kurla-cream flex items-center gap-2"><CreditCard className="w-4 h-4 text-kurla-amber" /> Encaissement SEPA — FR82 / BE76 (bornes payout)</h4>
        <p className="text-xs text-kurla-cream/60 leading-relaxed">
          Un seul compte Stripe suffit pour encaisser FR + BE en EUR (SEPA). Le « FR82 » et « BE76 » sont les <strong className="text-kurla-cream">scores pays</strong> (82/100 FR, 76/100 BE), pas deux IBAN obligatoires. L'IBAN de payout se configure <strong className="text-kurla-cream">dans Stripe Dashboard → Settings → Payouts</strong>, jamais dans l'app — l'app ne stocke pas d'IBAN complet.
        </p>
        <div className="grid sm:grid-cols-3 gap-3">
          <label className="text-xs text-kurla-cream/70 space-y-1 block">Titulaire (affiché facture)
            <input value={ibanDraft.titulaire} onChange={e=>persistIban({ ...ibanDraft, titulaire: e.target.value })} placeholder="KURLA Beauty SASU" className="w-full mt-1 px-3 py-2 rounded-xl bg-kurla-espresso border border-kurla-cream/15 text-xs text-kurla-cream focus:outline-none focus:border-kurla-copper" />
          </label>
          <label className="text-xs text-kurla-cream/70 space-y-1 block">IBAN FR — repère (masqué après saisie)
            <input value={ibanDraft.fr} onChange={e=>persistIban({ ...ibanDraft, fr: e.target.value })} placeholder="FR76 3000 4000 0500 0012 3456 789" className="w-full mt-1 px-3 py-2 rounded-xl bg-kurla-espresso border border-kurla-cream/15 text-xs text-kurla-cream font-mono focus:outline-none focus:border-kurla-copper" />
            <span className="text-[10px] text-kurla-cream/35">27 caractères FR14 + clé — vérifié côté Stripe</span>
          </label>
          <label className="text-xs text-kurla-cream/70 space-y-1 block">IBAN BE — si entité BE séparée
            <input value={ibanDraft.be} onChange={e=>persistIban({ ...ibanDraft, be: e.target.value })} placeholder="BE71 0961 2345 6789" className="w-full mt-1 px-3 py-2 rounded-xl bg-kurla-espresso border border-kurla-cream/15 text-xs text-kurla-cream font-mono focus:outline-none focus:border-kurla-copper" />
            <span className="text-[10px] text-kurla-cream/35">16 caractères BE71 — optionnel</span>
          </label>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={()=>copy(ibanMentions,'iban')} className="px-3 py-1.5 rounded-full bg-kurla-ink border border-kurla-cream/15 text-xs text-kurla-cream/70 flex items-center gap-1.5 hover:border-kurla-copper">
      ...
      {/* Suivi livraison */}
      <div className="p-4 rounded-2xl bg-kurla-ink border border-indigo-500/25 space-y-4">
        <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <div className="grid sm:grid-cols-3 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-kurla-espresso border border-kurla-cream/10">
            <p className="font-bold text-kurla-cream">1. Étiquette créée</p>
            <p className="text-kurla-cream/60 leading-relaxed mt-1"><code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">label_created</code> → commande <code>packed</code>. Numéro non exigé ici.</p>
          </div>
          <div className="p-3 rounded-xl bg-indigo-950/30 border border-indigo-500/20">
            ...
            <p className="text-kurla-cream/60 leading-relaxed mt-1"><code>shipped/in_transit/out_for_delivery</code> exigent <code>trackingNumber</code> non-fake (rejet <code>test/fake/dummy</code>). URL auto : La Poste / Mondial Relay / Chronopost / DHL.</p>
          </div>
          <div className="p-3 rounded-xl bg-emerald-950/20 border border-emerald-500/20">
            <p className="font-bold text-emerald-200">3. Livrée</p>
            <p className="text-kurla-cream/60 leading-relaxed mt-1"><code>delivered</code> → commande <code>delivered</code> + event <code>deliveredAt</code>. Historique <code>/api/shipments/:id/history</code> visible cliente.</p>
          </div>
        </div>
        ...
        {Object.keys(shipments).length===0 ? (
          <div className="p-4 rounded-xl bg-kurla-espresso border border-kurla-cream/10 text-center space-y-1">
            <p className="text-xs text-kurla-cream/60">Aucune expédition avec tracking enregistré — état honnête.</p>
            <p className="text-[11px] text-kurla-cream/40">Créez-en une via <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">Commandes → Expédier</code> (choisir transporteur + coller n° réel) → elle apparaîtra ici avec son lien de suivi.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse min-w-[760px]">
              <thead>
                <tr className="border-b border-kurla-cream/10 text-indigo-300 uppercase tracking-wider text-[10px]">
                  <th className="py-2 px-3">Commande</th>
              ...
              <tbody className="divide-y divide-kurla-cream/5">
                {Object.entries(shipments).map(([orderId, s])=>{
                  ...
                  return (
                    <tr key={orderId} className="hover:bg-kurla-espresso/60">
                      <td className="py-2.5 px-3 font-mono font-bold text-kurla-cream">{orderId}</td>
                      <td className="py-2.5 px-3"><span className="px-2 py-1 rounded-full bg-kurla-espresso border border-kurla-cream/10 text-kurla-cream/70 text-[11px]">{s.carrier || 'manual'}</span></td>
                      <td className="py-2.5 px-3 font-mono text-kurla-cream/80">{s.trackingNumber ? `${s.trackingNumber.slice(0,4)}****${s.trackingNumber.slice(-4)}` : <span className="text-amber-300">à saisir</span>}<span className="block text-[10px] text-kurla-cream/30 font-sans">{s.trackingNumber || ''}</span></td>
        ...
        <div className="p-3 rounded-xl bg-amber-950/20 border border-amber-500/20 text-xs text-amber-200 flex gap-2">
          ...
          <span><strong>Garde transporteur :</strong> un numéro commençant par <code className="px-1 py-0.5 rounded bg-kurla-ink border border-kurla-cream/10">test / fake / dummy / placeholder</code> est <strong>refusé par le serveur</strong> (<code>upsertShipment</code> lève <em>« valeur de test »</em>). Seul le numéro réel du bordereau fait passer la commande en <code>shipped</code> — pas d'invention possible.</span>
        </div>

        <p className="text-[11px] text-kurla-cream/35 leading-relaxed">
          Sources : <span className="text-kurla-amber">/api/admin/metrics</span> (stripeMode, paidOrders) + <span className="text-kurla-amber">/api/orders</span> (snapshot TVA : net/vat/total/vatCountry) + <span className="text-kurla-amber">/api/shipments/:orderId</span> (carrier/tracking/status/estimatedDelivery) · Logistique : <span className="text-kurla-amber">src/lib/countryFulfillment.ts</span> (FR 82/BE 76) + <span className="text-kurla-amber">src/lib/shippingRules.ts</span> (tarifs 4,90€/6,90€, gratuit dès 60/80€) + <span className="text-kurla-amber">src/lib/shippingService.ts</span> (URL tracking).
        </p>
      </div>
      ...
      <p className="text-[10px] text-kurla-cream/35 text-center leading-relaxed">C23 — bornes : <span className="text-kurla-amber">FR82/BE76</span> = scores <code>docs/sourcing/pays_scoring.md</code> · LIVE = <code>getStripeModeForCountry(FR|BE, sk_live)</code> · Facture = <code>server.ts /api/stripe/create-checkout-session</code> snapshot + <code>src/lib/checkoutVat.ts</code> · Suivi = <code>PATCH /api/admin/shipments/:orderId</code> → <code>upsertShipment</code> (garde anti-fake) → <code>updateOrderStatus</code>.</p>
    </div>
  );
};
