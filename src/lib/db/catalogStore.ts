...
export async function getProducts(store: SupabaseServerStore, options: { publishedOnly?: boolean; includeInactive?: boolean } = {}): Promise<any[]> {
    ...
    if (supabase) {
      ...
      const mapped = (data || []).map((p: any) => {
        ...
        inStock: p.is_preorder === true ? p.in_stock === true : p.in_stock === true && (productVariants.length > 0 ? variantAvailable : baseAvailable > 0),
        stockQuantity: p.is_preorder === true ? 0 : (baseStock ? Number(baseStock.quantity) : Number(p.stock_quantity || 0)),
        category: p.category,
