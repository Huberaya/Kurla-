...
import { ExternalLink, Info, Lock, Receipt, ShieldCheck, Target } from 'lucide-react';
...
export const BrandSpacePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setInvoiceError('');
    fetch('/api/brand-invoices/mine', { headers: { Authorization: `Bearer ${session.access_token}` } })
      .then(async response => {
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data?.error || 'Les factures n’ont pas pu être chargées.');
        if (!cancelled) setInvoices(Array.isArray(data.invoices) ? data.invoices : []);
      })
      .catch(error => { if (!cancelled) setInvoiceError(error?.message || 'Les factures n’ont pas pu être chargées.'); })
      .finally(() => { if (!cancelled) setInvoiceLoading(false); });
    return () => { cancelled = true; };
  }, [session?.access_token]);

  const payInvoice = async (invoiceId: string) => {
    if (!session?.access_token || payingInvoiceId) return;
    setPayingInvoiceId(invoiceId);
    setInvoiceError('');
    try {
      const response = await fetch(`/api/brand-invoices/${invoiceId}/checkout`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${session.access_token}` }
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.note || data?.error || 'Le paiement n’a pas pu être préparé.');
      if (data.checkoutUrl) window.location.assign(data.checkoutUrl);
      else throw new Error('Stripe n’a pas fourni d’URL de paiement.');
    } catch (error: any) {
      setInvoiceError(error?.message || 'Le paiement n’a pas pu être préparé.');
    } finally {
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 bg-[#050403] text-[#FFF7EF]">
      <div className="max-w-3xl mx-auto px-4">
        ...
        {/* --- Factures ---------------------------------------------------- */}
        {session && (
          <section className="mt-10 rounded-2xl border border-[#FFF7EF]/10 bg-[#FFF7EF]/[0.03] p-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold mb-2"><Receipt className="h-5 w-5 text-[#C8753D]" /> Mes factures</h2>
            <p className="mb-5 text-sm text-[#FFF7EF]/65">Les factures proviennent du contrat signé. Le montant n’est jamais modifiable depuis cet écran.</p>
            {invoiceError && <p className="mb-4 rounded-lg border border-[#C8753D]/30 bg-[#C8753D]/10 p-3 text-xs text-[#FFE0C6]">{invoiceError}</p>}
            {invoiceLoading ? <p className="text-sm text-[#FFF7EF]/55">Chargement des factures…</p> : invoices.length === 0 ? <p className="text-sm text-[#FFF7EF]/55">Aucune facture émise pour votre compte.</p> : <div className="space-y-3">{invoices.map(invoice => <article key={invoice.id} className="flex flex-col gap-3 rounded-xl border border-[#FFF7EF]/10 bg-[#050403] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">{invoice.invoiceNumber}</p><p className="mt-1 text-xs text-[#FFF7EF]/55">Émise le {new Date(invoice.issuedAt).toLocaleDateString('fr-FR')} · {(invoice.amountCents / 100).toFixed(2).replace('.', ',')} € {invoice.currency.toUpperCase()}</p><p className="mt-1 text-xs text-[#D49A63]">Statut : {invoice.status === 'pending' ? 'En attente de paiement' : invoice.status === 'paid' ? 'Réglée' : `Annulée${invoice.voidReason ? ` — ${invoice.voidReason}` : ''}`}</p></div>{invoice.status === 'pending' && <button type="button" disabled={payingInvoiceId === invoice.id} onClick={() => void payInvoice(invoice.id)} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#C8753D] px-4 py-2 text-xs font-semibold text-[#050403] disabled:opacity-60">{payingInvoiceId === invoice.id ? 'Préparation…' : <>Payer via Stripe <ExternalLink className="h-3.5 w-3.5" /></>}</button>}</article>)}</div>}
          </section>
        )}
