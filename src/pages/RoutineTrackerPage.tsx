...
export const RoutineTrackerPage: React.FC = () => {
  ...
  if (loading) return <div className="pt-32 min-h-screen text-center text-sm text-kurla-carbon/60"><Loader2 className="w-7 h-7 animate-spin text-kurla-copper mx-auto mb-3" />Chargement de ton calendrier synchronisé…</div>;
    ...
    <div className="pt-28 pb-24 bg-kurla-ivory text-kurla-carbon min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
          <div><div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-kurla-copper/10 text-kurla-copper text-xs font-semibold mb-2"><CalendarIcon className="w-3.5 h-3.5" /> Routine adaptative</div><h1 className="text-3xl sm:text-4xl font-serif-title font-bold">Mon calendrier qui apprend</h1><p className="text-sm text-kurla-carbon/70 font-light mt-2 max-w-2xl">Chaque tâche, observation et adaptation est liée à ton compte KURLA ID. Un autre appareil retrouvera le même historique après connexion.</p></div>
          <div className="flex flex-wrap gap-2"><button type="button" onClick={loadWeather} disabled={weatherLoading || !state?.plan} className="px-4 py-2.5 rounded-full bg-kurla-carbon text-white text-xs font-semibold inline-flex items-center gap-2 disabled:opacity-50"><MapPin className="w-4 h-4" />{weatherLoading ? 'Lecture météo…' : 'Adapter à ma météo'}</button><a href="/account/routine-id" className="px-4 py-2.5 rounded-full border border-kurla-stone text-xs font-semibold hover:border-kurla-copper">Paramètres</a></div>
        </header>
        ...
        {!state?.plan ? <section className="p-8 rounded-3xl bg-kurla-sand border border-kurla-stone text-center"><Sparkles className="w-8 h-8 text-kurla-copper mx-auto mb-3" /><h2 className="text-xl font-serif-title font-bold">Ta routine n’est pas encore configurée</h2><p className="text-sm text-kurla-carbon/65 mt-2">Décris tes moments, ton wash day, tes protections et ton temps disponible pour générer un calendrier persistant.</p><a href="/account/routine-id" className="inline-flex mt-5 px-5 py-3 rounded-full bg-kurla-copper text-white text-xs font-semibold">Configurer ma routine</a></section> : <>
          <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 p-6 rounded-3xl bg-kurla-carbon text-white"><div className="flex items-center justify-between gap-4"><div><p className="text-[10px] uppercase tracking-widest text-kurla-amber font-bold">Ajustements actifs</p><h2 className="text-xl font-serif-title font-bold mt-1">Pourquoi ce calendrier ?</h2></div><RefreshCw className="w-6 h-6 text-kurla-amber" /></div><div className="mt-4 space-y-2">{state.plan.adaptationNotes.length > 0 ? state.plan.adaptationNotes.map(note => <p key={note} className="text-xs text-white/80 leading-relaxed">• {note}</p>) : <p className="text-xs text-white/70">Aucune observation récente : la routine suit les paramètres de ton KURLA ID.</p>}</div></div>
            <div className="p-6 rounded-3xl bg-kurla-sand border border-kurla-stone"><div className="flex items-center gap-2"><CloudSun className="w-5 h-5 text-kurla-copper" /><h2 className="text-sm font-bold">Contexte météo</h2></div>{state.plan.weather ? <><p className="text-2xl font-bold mt-4">{state.plan.weather.temperatureC !== undefined ? `${Math.round(state.plan.weather.temperatureC)} °C` : 'Température non renseignée'}</p><p className="text-xs text-kurla-carbon/65 mt-1">{state.plan.weather.humidityPercent !== undefined ? `Humidité ${Math.round(state.plan.weather.humidityPercent)} %` : 'Humidité non renseignée'}{state.plan.weather.precipitationMm !== undefined ? ` · pluie ${Math.round(state.plan.weather.precipitationMm * 10) / 10} mm` : ''}</p><p className="text-[10px] text-kurla-carbon/50 mt-3">Source : {state.plan.weather.source || 'non renseignée'} · {state.plan.weather.observedAt ? new Date(state.plan.weather.observedAt).toLocaleString('fr-FR') : 'date non renseignée'}</p></> : <p className="text-xs text-kurla-carbon/60 mt-4">Aucune météo partagée. Utilise le bouton uniquement si tu veux autoriser l’accès à ta position.</p>}</div>
          </section>

          <section className="p-6 rounded-3xl bg-kurla-sand border border-kurla-stone"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5"><div><h2 className="text-base font-serif-title font-bold">Calendrier des gestes</h2><p className="text-xs text-kurla-carbon/60 mt-1">Les rappels sont générés pour les prochaines semaines et restent modifiables par ton retour.</p></div><span className="text-xs text-kurla-copper font-semibold">{state.tasks.filter(task => task.status === 'completed').length} / {state.tasks.length} réalisés</span></div><div className="space-y-4">{taskGroups.map(([date, tasks]) => <div key={date}><h3 className="text-xs uppercase tracking-wider font-bold text-kurla-copper mb-2">{formatDate(date)}</h3><div className="space-y-2">{tasks.map(task => <div key={task.id} className={`p-4 rounded-2xl border flex items-center justify-between gap-3 transition-colors ${task.status === 'completed' ? 'bg-emerald-50/60 border-emerald-200' : 'bg-kurla-ivory border-kurla-stone'}`}><button type="button" onClick={() => toggleTask(task)} disabled={busyTask === task.id} className="flex items-center gap-3 text-left min-w-0"><span className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${task.status === 'completed' ? 'bg-emerald-500 text-white' : 'border-2 border-kurla-stone'}`}>{busyTask === task.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className={`w-3.5 h-3.5 ${task.status === 'completed' ? '' : 'text-transparent'}`} />}</span><span className="min-w-0"><span className={`block text-xs font-semibold ${task.status === 'completed' ? 'line-through text-kurla-carbon/50' : ''}`}>{task.title}</span><span className="block text-[10px] text-kurla-carbon/55 mt-1">{kindLabels[task.kind]} · {task.durationMinutes} min{task.productLabels.length > 0 ? ` · déjà possédé : ${task.productLabels.join(', ')}` : ''}</span><span className="block text-[11px] text-kurla-carbon/65 mt-1">{task.description}</span></span></button><Bell className="w-4 h-4 text-kurla-carbon/25 shrink-0" /></div>)}</div></div>)}</div>{taskGroups.length === 0 && <p className="text-sm text-kurla-carbon/55">Aucun geste planifié avec ces paramètres.</p>}</section>

          <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone"><div className="flex items-start gap-3 mb-5"><MessageCircle className="w-5 h-5 text-kurla-copper shrink-0" /><div><h2 className="text-base font-serif-title font-bold">Boucle d’apprentissage</h2><p className="text-xs text-kurla-carbon/65 mt-1">Dis ce qui s’est réellement passé. La prochaine version de la routine sera ajustée et l’observation restera dans ton historique.</p></div></div><form onSubmit={submitFeedback} className="space-y-4"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">{signalOptions.map(option => <button key={option.value} type="button" onClick={() => setSignal(option.value)} className={`p-3 rounded-xl border text-left text-xs font-semibold transition-colors ${signal === option.value ? 'bg-kurla-copper text-white border-kurla-copper' : 'bg-kurla-sand border-kurla-stone hover:border-kurla-copper'}`}>{option.label}</button>)}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><input value={productLabel} onChange={event => setProductLabel(event.target.value)} maxLength={160} className="px-3.5 py-3 rounded-xl bg-kurla-sand border border-kurla-stone text-xs focus:outline-none focus:border-kurla-copper" placeholder="Produit concerné (facultatif)" /><textarea value={feedbackNote} onChange={event => setFeedbackNote(event.target.value)} maxLength={1000} className="px-3.5 py-3 rounded-xl bg-kurla-sand border border-kurla-stone text-xs min-h-[80px] resize-y focus:outline-none focus:border-kurla-copper" placeholder="Ce que tu as observé, quand et après quel geste…" /></div><button type="submit" disabled={savingFeedback} className="px-5 py-3 rounded-full bg-kurla-copper text-white text-xs font-semibold inline-flex items-center gap-2 disabled:opacity-60">{savingFeedback ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}Enregistrer et adapter</button></form></section>
