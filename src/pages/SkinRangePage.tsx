...
function FicheCible({ fiche }: { fiche: FicheCiblePeau }) {
  ...
  return (
    <article className="rounded-3xl border border-[#E8E1DA] bg-[#F8F2EC] p-5 flex flex-col transition-all hover:border-[#C8753D]/40">
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="text-[10px] px-2 py-1 rounded-full bg-white border border-[#E8E1DA] text-[#111111]/70 font-bold">
          {fiche.routineStep || fiche.subCategory}
        </span>
        <span className="inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-full bg-[#111111]/5 text-[#111111]/60 font-bold">
      ...
      {fiche.actifsAnnonces.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {fiche.actifsAnnonces.slice(0, 4).map(actif => (
            <span key={actif} className="text-[10px] px-2 py-0.5 rounded-full bg-[#C8753D]/10 text-[#C8753D] font-semibold">
              {actif}
            </span>
          ))}
        </div>
      ...
      <p className="text-xs text-[#111111]/60 font-light mt-2 leading-relaxed">{fiche.benefitPrimary}</p>

      <dl className="mt-3 space-y-1 text-[11px]">
        {[fiche.sizeLabel, fiche.texture, fiche.usageFrequency].filter(Boolean).length > 0 && (
          <div className="flex gap-1.5">
            <dt className="sr-only">Format</dt>
            <dd className="text-[#111111]/50">{[fiche.sizeLabel, fiche.texture, fiche.usageFrequency].filter(Boolean).join(' · ')}</dd>
          </div>
        )}
        {fiche.concerns.length > 0 && (
          <div className="flex gap-1.5">
            <dt className="text-[#111111]/40 shrink-0">Préoccupations</dt>
            <dd className="text-[#111111]/70">{fiche.concerns.map(c => libellerCle(c, LIBELLES_PREOCCUPATIONS)).join(', ')}</dd>
          </div>
        )}
        {fiche.skinTypes.length > 0 && (
          <div className="flex gap-1.5">
            <dt className="text-[#111111]/40 shrink-0">Types de peau</dt>
            <dd className="text-[#111111]/70">{fiche.skinTypes.map(t => libellerCle(t, LIBELLES_TYPES_PEAU)).join(', ')}</dd>
          </div>
        )}
        {fiche.forWho && (
          <div className="flex gap-1.5">
            <dt className="text-[#111111]/40 shrink-0">Pour</dt>
            <dd className="text-[#111111]/70">{fiche.forWho}</dd>
          </div>
        )}
        {fiche.notIdealIf && (
          <div className="flex gap-1.5">
            <dt className="text-[#111111]/40 shrink-0">À éviter si</dt>
            <dd className="text-[#111111]/70">{fiche.notIdealIf}</dd>
          </div>
        )}
      </dl>

      {fiche.howToUse && (
        <p className="text-[11px] text-[#111111]/55 mt-2 leading-relaxed">
          <span className="font-semibold">Emploi : </span>{fiche.howToUse}
        </p>
      )}

      {fiche.formuleCible && (
        <div className="mt-3 border-t border-[#E8E1DA] pt-2">
          <button
            type="button"
            onClick={() => setOuverte(!ouverte)}
            className="flex items-center justify-between w-full text-left text-[11px] font-bold text-[#111111]/70 hover:text-[#C8753D]"
          ...
          {ouverte && (
            <div className="mt-2">
              <p className="text-[10px] font-mono leading-relaxed text-[#111111]/60 break-words">{fiche.formuleCible}</p>
              <p className="text-[10px] text-[#111111]/45 mt-1.5 italic">
      ...
      <div className="mt-auto pt-4 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-[10px] text-[#111111]/45">prix cible, indicatif</p>
        </div>
        <button
          type="button"
          disabled
          aria-disabled="true"
          className="px-3 py-2 rounded-full bg-[#111111]/10 text-[#111111]/55 text-[11px] font-bold flex items-center gap-1.5 cursor-not-allowed"
        >
  ...
  return (
    <div className="min-h-screen pt-28 pb-24 bg-[#FFFDF9] text-[#111111]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <a href="/peau" className="inline-flex items-center gap-1.5 text-xs text-[#C8753D] font-semibold mb-4 hover:underline">
          <ArrowLeft className="w-4 h-4" /> Retour pôle peau
        </a>

        <div className="max-w-3xl mb-8">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#111111] text-[#D9A8A4] text-[10px] font-bold tracking-widest uppercase">
        ...
        <div className="rounded-2xl border border-[#C8753D]/30 bg-[#C8753D]/5 p-4 mb-10 flex gap-3">
          <AlertTriangle className="w-5 h-5 text-[#C8753D] shrink-0 mt-0.5" />
        ...
        {loading && <p className="text-sm text-[#111111]/50">Chargement des fiches…</p>}
        {error && (
          <p className="text-sm text-[#111111]/60">Les fiches de la gamme n’ont pas pu être chargées. Réessayez plus tard.</p>
        )}

        {!loading && !error && fiches.length === 0 && (
          <p className="text-sm text-[#111111]/60">Aucune fiche de formulation n’est publiée pour le moment.</p>
        )}
          ...
          return (
            <section key={groupe.id} className="mb-10">
              <div className="flex items-start gap-2.5 mb-3">
                <Icone className="w-4 h-4 text-[#C8753D] mt-1 shrink-0" />
                <div>
                  <h2 className="text-lg font-serif-title font-bold">{groupe.label}</h2>
                  <p className="text-xs text-[#111111]/60 font-light">{groupe.role}</p>
                </div>
                <span className="ml-auto text-[11px] text-[#111111]/45 shrink-0">{duGroupe.length}</span>
              </div>
        ...
        <section className="rounded-3xl border border-[#E8E1DA] bg-white p-6 sm:p-8">
          <h2 className="text-lg font-serif-title font-bold flex items-center gap-2">
            <Info className="w-4 h-4 text-[#C8753D]" /> Être prévenue quand c’est fabriqué
          </h2>
          <p className="text-xs text-[#111111]/70 font-light mt-2 leading-relaxed max-w-2xl">
          ...
          <div className="flex flex-wrap gap-3 mt-6 pt-5 border-t border-[#E8E1DA]">
            ...
            <a
              ...
              className="px-5 py-2.5 rounded-full border border-[#E8E1DA] text-xs font-bold hover:border-[#C8753D]"
            >
...
function FormulaireAttente() {
  ...
  if (etat === 'done') {
    return <p className="text-xs text-[#111111]/70">C’est noté. Nous écrirons quand la gamme sera fabriquée.</p>;
  }
  ...
  return (
    <form onSubmit={envoyer} className="flex flex-col sm:flex-row gap-2 max-w-md">
      <input
        ...
        className="flex-1 px-4 py-2.5 rounded-full border border-[#E8E1DA] bg-white text-sm focus:outline-none focus:border-[#C8753D]"
      />
      <button
        type="submit"
        disabled={etat === 'loading'}
        className="px-5 py-2.5 rounded-full bg-[#C8753D] text-white text-xs font-bold disabled:opacity-60"
      >
