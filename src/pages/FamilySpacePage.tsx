...
const inputClass = 'w-full px-3 py-2.5 rounded-xl bg-kurla-ivory border border-kurla-stone text-kurla-carbon text-xs placeholder:text-kurla-carbon/35 focus:outline-none focus:border-kurla-copper';
const buttonClass = 'inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-kurla-copper hover:bg-kurla-cocoa text-white text-xs font-semibold disabled:opacity-40';
const mutedButtonClass = 'inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-kurla-ivory border border-kurla-stone hover:border-kurla-copper text-kurla-carbon/75 text-xs font-semibold disabled:opacity-40';
...
export const FamilySpacePage: React.FC = () => {
  ...
  if (loading) return <div className="min-h-screen pt-40 bg-kurla-ivory text-kurla-carbon text-center text-sm">Chargement de l’espace famille…</div>;

  return (
    <div className="pt-28 pb-24 bg-kurla-ivory text-kurla-carbon min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        <header className="rounded-3xl bg-kurla-espresso text-kurla-cream p-8 sm:p-12 shadow-xl">
          <div className="max-w-3xl space-y-4">
            <span className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-kurla-amber font-bold"><Users className="w-4 h-4" /> KURLA Famille</span>
            <h1 className="text-3xl sm:text-5xl font-serif-title font-bold">Une plateforme pour toute la maison.</h1>
            <p className="text-sm sm:text-base text-kurla-cream/75 leading-relaxed">Des profils séparés pour les parents, les enfants et les ados. Des routines partagées quand c’est utile, sans mélanger les besoins ni les données.</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 text-xs">
              <div className="rounded-2xl bg-white/10 p-4"><Baby className="w-5 h-5 text-kurla-amber mb-2" /><strong className="block">Enfants protégés</strong><span className="text-kurla-cream/60">Tranches d’âge, consentement et produits documentés.</span></div>
              <div className="rounded-2xl bg-white/10 p-4"><Heart className="w-5 h-5 text-kurla-amber mb-2" /><strong className="block">Hommes visibles</strong><span className="text-kurla-cream/60">Barbe, waves, rasage, peau et cuir chevelu.</span></div>
              <div className="rounded-2xl bg-white/10 p-4"><CalendarDays className="w-5 h-5 text-kurla-amber mb-2" /><strong className="block">Rituels partagés</strong><span className="text-kurla-cream/60">Calendriers, coffrets et routines parent-enfant.</span></div>
            </div>
          </div>
        </header>
        ...
        <section className="grid lg:grid-cols-[1fr_380px] gap-6 items-start">
          <div className="space-y-4">
            <div><span className="text-xs uppercase tracking-widest text-kurla-copper font-bold">Profils séparés</span><h2 className="text-2xl font-serif-title font-bold mt-1">Qui prend soin de lui ou d’elle ?</h2><p className="text-sm text-kurla-carbon/65 mt-2">Utilisez un prénom ou un surnom. Pour un mineur, KURLA conserve uniquement une tranche d’âge et l’état du consentement.</p></div>
            <div className="grid sm:grid-cols-2 gap-4">
              {members.map(member => <article key={member.id} className="rounded-2xl border border-kurla-stone bg-kurla-sand p-5 space-y-3"><div className="flex justify-between gap-2"><div><h3 className="font-semibold">{member.displayName}</h3><p className="text-xs text-kurla-carbon/60">{familyAgeLabel(member.ageBand)}</p></div><span className="w-9 h-9 rounded-full bg-kurla-copper/15 flex items-center justify-center">{member.ageBand === 'adult' ? <Users className="w-4 h-4 text-kurla-copper" /> : <Baby className="w-4 h-4 text-kurla-copper" />}</span></div><div className="text-xs flex items-start gap-2">{member.ageBand === 'adult' ? <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" /> : member.consentStatus === 'granted' ? <Check className="w-4 h-4 text-emerald-600 shrink-0" /> : <LockKeyhole className="w-4 h-4 text-amber-600 shrink-0" />}<span>{member.ageBand === 'adult' ? 'Profil adulte séparé.' : member.consentStatus === 'granted' ? 'Consentement parental enregistré.' : 'Recommandations mineur verrouillées jusqu’au consentement.'}</span></div><div className="flex gap-2 pt-1"><button className={mutedButtonClass} onClick={() => { setEditingMemberId(member.id); editMember(member); }}>Modifier</button>{member.ageBand !== 'adult' && member.consentStatus === 'granted' && <button className={mutedButtonClass} onClick={() => revokeConsent(member)} disabled={busy === `revoke-${member.id}`}>Retirer le consentement</button>}<button aria-label={`Supprimer ${member.displayName}`} className="p-2 rounded-xl border border-rose-200 text-rose-600" onClick={() => run(`delete-${member.id}`, `/api/family/members/${member.id}`, { method: 'DELETE' }, 'Profil supprimé.')}><Trash2 className="w-3.5 h-3.5" /></button></div></article>)}
              {!members.length && <div className="sm:col-span-2 p-6 rounded-2xl border border-dashed border-kurla-stone text-sm text-kurla-carbon/55">Ajoutez le premier profil pour créer une routine ou un calendrier familial.</div>}
            </div>
          </div>

          <form className="p-5 rounded-2xl bg-kurla-sand border border-kurla-stone space-y-3" onSubmit={saveMember}><h3 className="font-semibold flex items-center gap-2"><Plus className="w-4 h-4 text-kurla-copper" /> {editingMemberId ? 'Modifier un profil' : 'Ajouter un profil'}</h3><input className={inputClass} required placeholder="Prénom ou surnom" value={memberForm.displayName} onChange={event => setMemberForm({ ...memberForm, displayName: event.target.value })} /><div className="grid grid-cols-2 gap-2"><select className={inputClass} value={memberForm.profileKind} onChange={event => setMemberForm({ ...memberForm, profileKind: event.target.value, ageBand: event.target.value === 'adult' ? 'adult' : 'child' })}><option value="child">Enfant / adolescent</option><option value="adult">Adulte</option></select><select className={inputClass} value={memberForm.ageBand} disabled={memberForm.profileKind === 'adult'} onChange={event => setMemberForm({ ...memberForm, ageBand: event.target.value })}>{FAMILY_AGE_BANDS.filter(age => memberForm.profileKind === 'adult' ? age === 'adult' : age !== 'adult').map(age => <option key={age} value={age}>{FAMILY_AGE_LABELS[age]}</option>)}</select></div><select className={inputClass} value={memberForm.focus} onChange={event => setMemberForm({ ...memberForm, focus: event.target.value })}><option value="">Besoin principal non renseigné</option><option value="demelage">Démêlage et douceur</option><option value="cuir_chevelu">Confort du cuir chevelu</option><option value="barbe">Barbe et rasage</option><option value="waves">Waves et cheveux courts</option><option value="soleil">Protection solaire</option></select>{memberForm.profileKind === 'child' && <label className="flex items-start gap-2 p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-900"><input type="checkbox" checked={memberForm.parentalConsent === true} onChange={event => setMemberForm({ ...memberForm, parentalConsent: event.target.checked })} className="mt-0.5" /><span>Je suis le parent ou représentant légal et j’autorise KURLA à utiliser ce profil pour proposer des routines adaptées. Aucun âge exact, nom complet ou photo d’enfant n’est demandé.</span></label>}<div className="flex gap-2"><button className={buttonClass} disabled={busy === 'member'}><Save className="w-3.5 h-3.5" /> Enregistrer</button>{editingMemberId && <button type="button" className={mutedButtonClass} onClick={() => { setEditingMemberId(null); setMemberForm(emptyMember()); }}><X className="w-3.5 h-3.5" /> Annuler</button>}</div></form>
        </section>

        <section className="grid lg:grid-cols-[1fr_420px] gap-6 items-start">
          <div className="space-y-4"><div><span className="text-xs uppercase tracking-widest text-kurla-copper font-bold">Routines, cadeaux, calendrier</span><h2 className="text-2xl font-serif-title font-bold mt-1">Les rituels qui vivent ensemble.</h2><p className="text-sm text-kurla-carbon/65 mt-2">Une routine active impliquant un mineur ne peut utiliser que des produits dont l’âge, la sécurité mineur et l’image sont documentés.</p></div><div className="space-y-3">{plans.map(plan => <article key={plan.id} className="rounded-2xl border border-kurla-stone bg-white p-5"><div className="flex justify-between gap-3"><div><span className="text-[11px] uppercase tracking-wider text-kurla-copper">{familyPlanLabel(plan.planType)} · {plan.status}</span><h3 className="font-semibold mt-1">{plan.title}</h3></div><div className="flex gap-2"><button className={mutedButtonClass} onClick={() => { setEditingPlanId(plan.id); editPlan(plan); }}>Modifier</button><button className="p-2 rounded-xl border border-rose-200 text-rose-600" aria-label={`Supprimer ${plan.title}`} onClick={() => run(`delete-plan-${plan.id}`, `/api/family/plans/${plan.id}`, { method: 'DELETE' }, 'Plan supprimé.')}><Trash2 className="w-3.5 h-3.5" /></button></div></div><p className="text-xs text-kurla-carbon/60 mt-2">{plan.audience === 'shared' ? 'Tous les profils' : `${plan.memberIds.length} profil(s) sélectionné(s)`} · {plan.productIds.length} produit(s) associé(s) · {plan.schedule.length} entrée(s) calendrier</p>{plan.notes && <p className="text-xs text-kurla-carbon/60 mt-2">{plan.notes}</p>}</article>)}{!plans.length && <div className="p-6 rounded-2xl border border-dashed border-kurla-stone text-sm text-kurla-carbon/55">Aucun plan familial. Commencez par un calendrier de lavage, un coffret cadeau ou une routine parent-enfant.</div>}</div></div>

          <form className="p-5 rounded-2xl bg-kurla-sand border border-kurla-stone space-y-3" onSubmit={savePlan}><h3 className="font-semibold flex items-center gap-2"><CalendarDays className="w-4 h-4 text-kurla-copper" /> {editingPlanId ? 'Modifier un plan' : 'Créer un plan familial'}</h3><input className={inputClass} required placeholder="Nom du plan" value={planForm.title} onChange={event => setPlanForm({ ...planForm, title: event.target.value })} /><div className="grid grid-cols-2 gap-2"><select className={inputClass} value={planForm.planType} onChange={event => setPlanForm({ ...planForm, planType: event.target.value })}>{Object.entries(FAMILY_PLAN_LABELS).map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select><select className={inputClass} value={planForm.status} onChange={event => setPlanForm({ ...planForm, status: event.target.value })}><option value="draft">Brouillon</option><option value="active">Actif</option><option value="archived">Archivé</option></select></div><select className={inputClass} value={planForm.audience} onChange={event => setPlanForm({ ...planForm, audience: event.target.value })}><option value="shared">Tous les profils</option><option value="selected">Profils sélectionnés</option></select>{planForm.audience === 'selected' && <div className="space-y-2">{members.map(member => <label key={member.id} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={planForm.memberIds.includes(member.id)} onChange={() => togglePlanMember(member.id)} />{member.displayName} · {familyAgeLabel(member.ageBand)}</label>)}</div>}<div className="p-3 rounded-xl bg-white border border-kurla-stone text-xs"><p className="font-semibold mb-2">Profils concernés</p>{selectedPlanMembers.length ? selectedPlanMembers.map(member => <span key={member.id} className="inline-block mr-2 mb-1 px-2 py-1 rounded-full bg-kurla-sand">{member.displayName}</span>) : <span className="text-kurla-carbon/50">Aucun profil pour le moment</span>}</div><div className="p-3 rounded-xl bg-white border border-kurla-stone text-xs space-y-2"><p className="font-semibold flex items-center gap-2">Produits associés <span className="font-normal text-kurla-carbon/50">({familyProducts.length} documentés pour les profils concernés)</span></p>{familyProducts.length > 0 ? <div className="max-h-40 overflow-y-auto space-y-2">{familyProducts.slice(0, 40).map(product => <label key={product.id} className="flex items-start gap-2"><input type="checkbox" checked={planForm.productIds.includes(product.id)} onChange={() => togglePlanProduct(product.id)} /><span>{product.name}<span className="block text-[10px] text-kurla-carbon/50">{product.recommendedAgeBand || 'âge recommandé non renseigné'}{product.minorSafetyStatus === 'verified' ? ' · sécurité mineur documentée' : ''}</span></span></label>)}</div> : <p className="text-kurla-carbon/50">Aucun produit publié et documenté pour les profils mineurs sélectionnés.</p>}</div><textarea className={inputClass} rows={3} placeholder="Calendrier, une entrée par ligne : 2026-09-01 | Lavage doux | id-profil" value={planForm.scheduleText} onChange={event => setPlanForm({ ...planForm, scheduleText: event.target.value })} /><textarea className={inputClass} rows={3} placeholder="Notes ou occasion du cadeau" value={planForm.notes} onChange={event => setPlanForm({ ...planForm, notes: event.target.value })} /><div className="flex gap-2"><button className={buttonClass} disabled={busy === 'plan'}>{planForm.planType === 'gift' ? <Gift className="w-3.5 h-3.5" /> : <Save className="w-3.5 h-3.5" />} Enregistrer le plan</button>{editingPlanId && <button type="button" className={mutedButtonClass} onClick={() => { setEditingPlanId(null); setPlanForm(emptyPlan(space?.id || '')); }}>Nouveau</button>}</div></form>
        </section>

        <section className="rounded-2xl bg-kurla-espresso text-kurla-cream p-6 flex gap-3 items-start"><ShieldCheck className="w-5 h-5 text-kurla-amber shrink-0 mt-0.5" /><div><h2 className="font-semibold">Notre règle famille</h2><p className="text-xs text-kurla-cream/65 mt-1 leading-relaxed">KURLA ne demande pas de date de naissance exacte ni de photo d’enfant pour faire fonctionner cet espace. Les données restent séparées par profil, le consentement peut être retiré et les produits non documentés pour les mineurs ne sont pas proposés dans une routine active.</p></div></section>
      </div>
    </div>
  );
};
