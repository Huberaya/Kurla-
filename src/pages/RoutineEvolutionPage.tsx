...
import { applyRoutineEvolution, getProgressJournal, getRoutineEvolution } from '../services/routineService';
import type { HairEvolutionReport } from '../lib/knowledge/profileEvolution';
import { JOURNAL_SIGNAL_LABELS } from '../lib/knowledge/profileEvolution';

/**
 * D2 — « VOTRE PROFIL A ÉVOLUÉ ». Le pont entre le journal de progression et
 * le moteur segmenté : ce que le journal a dit, ce que KURLA en déduit, la
 * routine re-exécutée avec la cause de chaque changement. L'application au
 * profil est un choix explicite de l'utilisateur — KURLA ne réécrit jamais
 * un profil en silence.
 */

type JournalEntry = { id: string; entryDate: string; signals: string[]; note?: string };
...
export const RoutineEvolutionPage: React.FC = () => {
  ...
  const [report, setReport] = useState<HairEvolutionReport | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([getRoutineEvolution(token), getProgressJournal(token)])
      .then(([evolution, journal]) => {
        if (cancelled) return;
        setReport((evolution.report ?? null) as HairEvolutionReport | null);
        setApplied(Boolean(evolution.applied));
        setEntries((journal.journal ?? []) as JournalEntry[]);
      })
  ...
  const apply = async () => {
    ...
    setError('');
    try {
      const result = await applyRoutineEvolution(token);
      setReport((result.report ?? null) as HairEvolutionReport | null);
      setApplied(true);
    } catch (err) {
  ...
  if (loading) return <div className={shell}><div className="mx-auto flex max-w-3xl items-center gap-3 py-20 text-sm text-kurla-carbon/60"><Loader2 className="w-5 h-5 animate-spin text-kurla-copper" />Le moteur relit ton journal…</div></div>;

  const unavailable = !report?.available;
    ...
    <div className={shell}>
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <a href="/account/progress" className="inline-flex items-center gap-2 text-sm font-semibold text-kurla-copper hover:underline">Tenir mon journal <Calendar className="w-4 h-4" /></a>
        </div>
        ...
        <header className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone">
          ...
          <h1 className="mt-3 text-2xl font-serif-title font-bold">Ce que ton journal a changé dans ta routine</h1>
          <p className="mt-2 text-sm text-kurla-carbon/65 leading-relaxed">
            Tu as noté tes observations ; le moteur KURLA les a relues et a recalculé ta routine avec.
            Rien n’est modifié dans ton profil sans ton accord — tu lis, tu compares, tu appliques.
          </p>
        ...
        {unavailable ? (
          <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone text-sm text-kurla-carbon/70 space-y-4">
            <p>{report?.whyUnavailable ?? 'La boucle attend un diagnostic enregistré.'}</p>
            <div className="flex flex-wrap gap-3">
              <a href="/diagnostic" className="rounded-full bg-kurla-copper px-5 py-2.5 text-sm font-bold text-white hover:bg-kurla-cocoa">Faire un diagnostic</a>
              <a href="/account/progress" className="rounded-full border border-kurla-stone px-5 py-2.5 text-sm font-semibold hover:border-kurla-copper">Ouvrir mon journal</a>
            </div>
          </section>
        ) : (
          <>
            <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone space-y-4">
              <h2 className="text-lg font-serif-title font-bold">1 · Ce que ton journal a dit</h2>
              ...
              {entries.length === 0 && <p className="text-kurla-carbon/55">Aucune entrée pour le moment — note une observation après ton prochain lavage pour nourrir la boucle.</p>}
            </section>

            <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone space-y-4">
              <h2 className="text-lg font-serif-title font-bold">2 · Ce que KURLA en déduit</h2>
              ...
              {confirmations.length > 0 && (
                <div className="space-y-2 pt-1">
                  <h3 className="text-xs font-bold uppercase tracking-wide text-kurla-carbon/55">Ce qui ne change pas — et pourquoi</h3>
            ...
            <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone space-y-4">
              <h2 className="text-lg font-serif-title font-bold">3 · Ta routine recalculée</h2>
              <p className="text-xs text-kurla-carbon/60">Le moteur segmenté a été re-exécuté avec tes observations. « Nouveau » = l’étape est apparue ; « recalée » = l’étape reste mais son geste ou son pourquoi a changé à cause du journal.</p>
              <div className="grid gap-3 sm:grid-cols-3">
                {([['Jour de lavage', 'morning'], ['Entre deux lavages', 'evening'], ['Chaque semaine', 'weekly']] as const).map(([label, key]) => (
                  <div key={key} className="p-4 rounded-2xl bg-kurla-sand space-y-2">
                    <h3 className="text-[11px] font-bold uppercase tracking-wide text-kurla-carbon/55">{label}</h3>
              ...
              {report.removed.length > 0 && (
                <p className="text-xs text-kurla-carbon/60">Retirées cette fois : {report.removed.join(' · ')}.</p>
              )}
              <details className="text-xs text-kurla-carbon/70">
                ...
                <div className="mt-3 space-y-1 leading-relaxed">
                  <p><strong>Avant — jour de lavage :</strong> {report.before.morning.join(' · ') || '—'}</p>
                  <p><strong>Avant — entre deux :</strong> {report.before.evening.join(' · ') || '—'}</p>
            ...
            <section className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                <h2 className="text-lg font-serif-title font-bold">4 {applied ? '· Appliquée' : '· Valider dans ton profil'}</h2>
                <p className="mt-1 text-xs leading-relaxed text-kurla-carbon/65">
                  {applied
                    ...
                    : 'En appliquant, seul le profil cosmétique change (besoin, cuir chevelu, dosage) — rien d’autre n’est collecté.'}
                </p>
              </div>
              {!applied && changes.length > 0 && (
                <button type="button" onClick={apply} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-full bg-kurla-copper px-6 py-3 text-sm font-bold text-white hover:bg-kurla-cocoa disabled:opacity-60">
        ...
        <p className="text-[10px] text-kurla-carbon/45 leading-relaxed">Conseil cosmétique : l’évolution recalcule une routine de soin à partir de tes observations déclarées. Elle ne pose pas de diagnostic et ne remplace pas un avis médical.</p>
      </div>
    </div>
  );
};
