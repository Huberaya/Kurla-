    ...
    <div className="min-h-screen bg-[#FAF7F2] py-12 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-full bg-kurla-copper/10 text-kurla-copper flex items-center justify-center mx-auto mb-4">
            <Package className="w-7 h-7" />
          </div>
          <h1 className="text-2xl md:text-3xl font-serif-title font-bold text-kurla-carbon">Suivre ma commande</h1>
          <p className="text-sm text-kurla-carbon/60 mt-2">Entre ton numéro de commande (commence par <span className="font-mono font-semibold">ORD-…</span>) et l’email utilisé lors de l’achat.</p>
        </div>

        <form onSubmit={track} className="bg-white rounded-2xl border border-kurla-stone p-5 shadow-sm space-y-4">
          <div>
            <label className="block text-xs font-semibold text-kurla-carbon mb-1.5">Numéro de commande</label>
            <div className="relative">
              <Search className="w-4 h-4 text-kurla-carbon/30 absolute left-3 top-1/2 -translate-y-1/2" />
              <input
                value={orderInput}
                onChange={e => setOrderInput(e.target.value)}
                placeholder="ORD-XXXXXXXX"
                className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-kurla-stone text-sm font-mono focus:outline-none focus:border-kurla-copper uppercase"
              />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-kurla-carbon mb-1.5">Email de la commande</label>
            <input
              type="email"
              value={email}
              onChange={e => setEmail(e.target.value)}
              placeholder="ton.email@exemple.com"
              className="w-full px-3 py-2.5 rounded-xl border border-kurla-stone text-sm focus:outline-none focus:border-kurla-copper"
            />
          </div>
          {error && (
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 rounded-full bg-kurla-copper hover:bg-kurla-cocoa disabled:opacity-50 text-white text-sm font-semibold flex items-center justify-center gap-2 transition-colors"
          >
        ...
        {order && (
          <div className="mt-6 space-y-4">
            {/* En-tête + statut */}
            <div className="bg-white rounded-2xl border border-kurla-stone p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div>
                  <p className="text-xs text-kurla-carbon/50">Commande</p>
                  <p className="font-mono font-bold text-kurla-carbon">{order.id}</p>
                  <p className="text-xs text-kurla-carbon/50 mt-1">Passée le {formatDate(order.createdAt)}</p>
                </div>
                <span className={`px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 ${
                  ...
                  : 'bg-kurla-copper/10 text-kurla-copper border border-kurla-copper/25'
                }`}>
            ...
            {stepIdx >= 0 && !isProblem && (
              <div className="bg-white rounded-2xl border border-kurla-stone p-5 shadow-sm">
                <h2 className="text-sm font-bold text-kurla-carbon mb-5">Progression de la commande</h2>
                    ...
                    return (
                      <li key={step.key} className="flex gap-3 relative">
                        {i < STATUS_FLOW.length - 1 && (
                          <span className={`absolute left-[15px] top-8 w-0.5 h-6 ${i < stepIdx ? 'bg-kurla-copper' : 'bg-kurla-stone'}`} />
                        )}
                        <span className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                          done ? 'bg-kurla-copper text-white' : 'bg-[#F2EDE7] text-kurla-carbon/30'
                        } ${current ? 'ring-4 ring-kurla-copper/20' : ''}`}>
                          {done ? <CheckCircle2 className="w-4 h-4" /> : step.icon}
                        </span>
                        <div className="pt-1">
                          <p className={`text-sm font-semibold ${done ? 'text-kurla-carbon' : 'text-kurla-carbon/40'}`}>{step.label}</p>
                          <p className="text-xs text-kurla-carbon/50">{step.desc}</p>
                          {current && i === 0 && <p className="text-[11px] text-kurla-copper mt-0.5">{formatDateTime(order.createdAt)}</p>}
                        </div>
                      </li>
                    );
                  })}
            ...
            {order.status === 'shipped' && (
              <div className="bg-white rounded-2xl border border-kurla-stone p-5 shadow-sm">
                <h2 className="text-sm font-bold text-kurla-carbon mb-3 flex items-center gap-2"><Truck className="w-4 h-4 text-kurla-copper" /> Expédition</h2>
                {order.carrier && <p className="text-xs text-kurla-carbon/70 mb-1">Transporteur : <strong>{order.carrier}</strong></p>}
                {order.trackingNumber && (
                  <p className="text-xs text-kurla-carbon/70 mb-2">Numéro de suivi : <span className="font-mono font-semibold">{order.trackingNumber}</span></p>
                )}
                {order.trackingUrl ? (
                  ...
                  <p className="text-xs text-kurla-carbon/50">Le lien de suivi sera disponible dès la prise en charge par le transporteur.</p>
                )}
              </div>
            )}
            ...
            {/* Articles */}
            <div className="bg-white rounded-2xl border border-kurla-stone p-5 shadow-sm">
              <h2 className="text-sm font-bold text-kurla-carbon mb-3 flex items-center gap-2"><ShoppingBag className="w-4 h-4 text-kurla-copper" /> Articles ({order.items.reduce((n, i) => n + (i.quantity || 1), 0)})</h2>
              <ul className="divide-y divide-[#F2EDE7]">
                {order.items.map((item, i) => (
                  <li key={i} className="py-2.5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm text-kurla-carbon truncate">
                        {item.slug ? <a href={`/produit/${item.slug}`} className="hover:underline">{item.name}</a> : item.name}
                      </p>
                      <p className="text-[11px] text-kurla-carbon/50">Qté {item.quantity}{item.isPreorder ? ' · Précommande' : ''}</p>
                    </div>
                    <span className="text-sm font-semibold text-kurla-carbon shrink-0">{item.amount != null ? formatMoney(item.amount) : ''}</span>
                  </li>
                ))}
              </ul>
              <div className="border-t border-[#F2EDE7] mt-2 pt-3 flex justify-between text-sm">
                <span className="text-kurla-carbon/60">Total</span>
                <span className="font-bold text-kurla-carbon">{formatMoney(order.total)}</span>
              </div>
            </div>

            {/* Retour / aide */}
            <div className="bg-white rounded-2xl border border-kurla-stone p-4 shadow-sm flex items-start gap-2">
              <RotateCcw className="w-4 h-4 text-kurla-carbon/40 mt-0.5 shrink-0" />
              <p className="text-xs text-kurla-carbon/60">
