...
export const ProgressJournalPage: React.FC = () => {
  ...
  if (loading) return <div className="pt-32 min-h-screen text-center text-sm text-kurla-carbon/60"><Loader2 className="w-7 h-7 animate-spin text-kurla-copper mx-auto mb-3" />Chargement de ton journal persistant…</div>;
  ...
  return (
    <div className="pt-28 pb-24 bg-kurla-ivory text-kurla-carbon min-h-screen">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-kurla-sand border border-kurla-stone space-y-6"><div className="flex items-start gap-3"><Calendar className="w-5 h-5 text-kurla-copper shrink-0" /><div><h2 className="text-xl font-serif-title font-bold">Ajouter une observation</h2><p className="text-xs text-kurla-carbon/65 mt-1">Tu peux ne remplir que ce que tu as réellement observé.</p></div></div><form onSubmit={saveEntry} className="space-y-5"><div><label className="block text-xs font-bold mb-1.5">Date de l’observation</label><input type="date" value={entryDate} onChange={event => setEntryDate(event.target.value)} className={fieldClass} required /></div><div><span className="block text-xs font-bold mb-2">Résultat ou signal observé</span><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{signalOptions.map(item => <button type="button" key={item.value} onClick={() => toggleSignal(item.value)} className={`p-3 rounded-xl border text-left text-xs font-semibold transition-colors ${selectedSignals.includes(item.value) ? 'bg-kurla-copper text-white border-kurla-copper' : 'bg-kurla-ivory border-kurla-stone hover:border-kurla-copper'}`}>{item.label}</button>)}</div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{([['hydrationScore', 'Hydratation / souplesse'], ['breakageScore', 'Casse observée'], ['comfortScore', 'Confort peau / cuir chevelu'], ['detanglingScore', 'Facilité de démêlage']] as const).map(([key, label]) => <div key={key}><label className="block text-xs font-bold mb-1.5">{label}</label><select value={metrics[key] || ''} onChange={event => setMetrics(current => ({ ...current, [key]: event.target.value }))} className={fieldClass}>{metricOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>)}</div><div><label className="block text-xs font-bold mb-1.5">Ce que tu as remarqué</label><textarea value={note} onChange={event => setNote(event.target.value)} maxLength={3000} className={`${fieldClass} min-h-[110px] resize-y`} placeholder="Ex. après le wash day, mes longueurs restent souples mais le cuir chevelu tiraille…" /></div><div><label className="block text-xs font-bold mb-1.5">Produits utilisés (facultatif)</label><textarea value={productsUsed} onChange={event => setProductsUsed(event.target.value)} className={`${fieldClass} min-h-[70px] resize-y`} placeholder="Un produit par ligne ou séparé par des virgules" /></div><button type="submit" disabled={saving} className="w-full py-4 rounded-full bg-kurla-copper hover:bg-kurla-cocoa text-white text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-60">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}{saving ? 'Enregistrement et recalcul…' : 'Enregistrer dans mon journal'}</button></form></div>

        <section className="space-y-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-serif-title font-bold">Historique</h2><p className="text-xs text-kurla-carbon/60 mt-1">{journal.length} observation{journal.length > 1 ? 's' : ''} · {persistence === 'supabase' ? 'synchronisé avec ton compte' : 'stockage serveur de développement'}</p></div><div className="flex items-center gap-3"><a href="/account/routine-evolution" className="text-xs font-bold text-kurla-copper hover:underline">Routine recalculée →</a><ShieldCheck className="w-5 h-5 text-kurla-copper" /></div></div>{journal.length === 0 ? <div className="p-8 rounded-3xl bg-kurla-ivory border border-kurla-stone text-sm text-kurla-carbon/55">Aucune observation enregistrée pour le moment.</div> : journal.map(entry => <article key={entry.id} className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone shadow-xs"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2"><div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-kurla-copper" /><time className="text-sm font-bold">{new Date(`${entry.entryDate}T12:00:00`).toLocaleDateString('fr-FR', { dateStyle: 'long' })}</time></div><span className="text-[10px] text-kurla-carbon/50">Enregistré le {new Date(entry.createdAt).toLocaleString('fr-FR')}</span></div>{entry.signals.length > 0 && <div className="flex flex-wrap gap-2 mt-4">{entry.signals.map(signal => <span key={signal} className="px-2.5 py-1 rounded-full bg-kurla-copper/10 text-kurla-copper text-[10px] font-semibold">{signalOptions.find(item => item.value === signal)?.label || signal}</span>)}</div>}<div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">{[['Hydratation', 'hydrationScore'], ['Casse', 'breakageScore'], ['Confort', 'comfortScore'], ['Démêlage', 'detanglingScore']].map(([label, key]) => <div key={key} className="p-3 rounded-xl bg-kurla-sand text-xs"><span className="block text-[10px] text-kurla-carbon/55">{label}</span><strong>{metricValue(entry, key as 'hydrationScore' | 'breakageScore' | 'comfortScore' | 'detanglingScore')}</strong></div>)}</div>{entry.note && <p className="text-sm text-kurla-carbon/75 leading-relaxed mt-4 whitespace-pre-wrap">{entry.note}</p>}{entry.productsUsed.length > 0 && <p className="text-xs text-kurla-carbon/55 mt-3">Produits utilisés : {entry.productsUsed.join(', ')}</p>}</article>)}</section>
