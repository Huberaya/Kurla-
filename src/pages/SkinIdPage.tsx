...
export const SkinIdPage: React.FC = () => {
  ...
  if (loading) return <div className="pt-32 pb-24 bg-[#FFFDF9] min-h-screen flex items-center justify-center"><span className="text-sm text-[#111111]/60">Chargement de Ma peau…</span></div>;
  ...
  return (
    <div className="pt-28 pb-24 bg-[#FFFDF9] text-[#111111] min-h-screen">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <a href="/account/kurla-id" className="inline-flex items-center gap-1.5 text-xs text-[#C8753D] font-semibold hover:underline"><ArrowLeft className="w-4 h-4" /> Retour KURLA ID</a>

        {/* Header Ma peau */}
        <div className="p-6 sm:p-8 rounded-3xl bg-[#F8F2EC] border border-[#E8E1DA] flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-widest font-bold text-[#C8753D] flex items-center gap-1.5"><Heart className="w-3.5 h-3.5" /> Ma peau · KURLA SKIN</p>
            <h1 className="text-2xl sm:text-3xl font-serif-title font-bold mt-1">Votre peau, suivie dans le temps</h1>
            <p className="text-sm text-[#111111]/65 font-light mt-2 max-w-2xl">Profil peau + journal. Tout est modifiable, tout peut rester “Je ne sais pas”. Aucune donnée n’est revendue — export ou suppression en un clic dans <a href="/account/donnees" className="underline">Vos données</a>.</p>
            {!hasData && <p className="mt-3 text-xs"><a href="/peau/diagnostic" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#111111] text-white font-bold">Passer le diagnostic peau (2 min) →</a></p>}
          </div>
          <div className="min-w-[180px] p-4 rounded-2xl bg-[#FFFDF9] border border-[#E8E1DA] text-center">
            <span className="text-[10px] uppercase tracking-wider text-[#111111]/55 block">Complétude peau</span>
            <span className="text-3xl font-bold text-[#C8753D]">{confidence.skin}%</span>
            <span className="text-[10px] text-[#111111]/55 block">{confidence.knownFields}/{confidence.totalFields} champs</span>
            <a href="/peau/diagnostic" className="mt-2 inline-block text-[11px] font-bold text-[#C8753D] hover:underline">Modifier le diagnostic</a>
          </div>
        </div>
        ...
        {/* Recap peau */}
        <div className="p-6 sm:p-8 rounded-3xl bg-[#FFFDF9] border border-[#E8E1DA] space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold flex items-center gap-2"><Droplets className="w-5 h-5 text-[#C8753D]" /> Profil peau actuel</h2>
            <a href="/peau/diagnostic" className="text-xs font-bold text-[#C8753D] hover:underline inline-flex items-center gap-1"><Edit3 className="w-3.5 h-3.5" /> Refaire le diagnostic</a>
          </div>
          {!hasData ? (
            <p className="text-sm text-[#111111]/60">Aucune donnée peau enregistrée. Lancez le diagnostic pour voir votre recap ici et obtenir une routine.</p>
          ) : (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]"><p className="text-[10px] uppercase font-bold text-[#C8753D]">Type</p><p className="font-semibold mt-1">{label(skin.skinType)}</p></div>
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]"><p className="text-[10px] uppercase font-bold text-[#C8753D]">Carnation</p><p className="font-semibold mt-1">{label(skin.toneDepth)}</p><p className="text-[11px] text-[#111111]/50">{label(skin.undertone)}</p></div>
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]"><p className="text-[10px] uppercase font-bold text-[#C8753D]">Hydratation</p><p className="font-semibold mt-1">{label(skin.hydrationLevel || skin.hydration)}</p><p className="text-[11px] text-[#111111]/50">HPI · {label(skin.hyperpigmentationTendency)}</p></div>
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]"><p className="text-[10px] uppercase font-bold text-[#C8753D]">Budget</p><p className="font-semibold mt-1">{label(skin.budget)}</p><p className="text-[11px] text-[#111111]/50">{label(skin.ageRange)}</p></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]">
                  <p className="font-bold text-[#C8753D] mb-1">Préoccupations</p>
                  <div className="flex flex-wrap gap-1.5">{(skin.skinConcerns || []).filter(v => v !== UNKNOWN).slice(0, 5).map(v => <span key={v} className="px-2 py-1 rounded-full bg-white border border-[#E8E1DA] text-[11px] font-semibold">{label(v)}</span>)}{(skin.skinConcerns || []).filter(v => v !== UNKNOWN).length === 0 && <span className="text-[#111111]/50">—</span>}</div>
                </div>
                <div className="p-3 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA]">
                  <p className="font-bold text-[#C8753D] mb-1">Objectifs</p>
                  <div className="flex flex-wrap gap-1.5">{(skin.skinObjectives || []).filter(v => v !== UNKNOWN).slice(0, 5).map(v => <span key={v} className="px-2 py-1 rounded-full bg-white border border-[#E8E1DA] text-[11px] font-semibold">{label(v)}</span>)}{(skin.skinObjectives || []).filter(v => v !== UNKNOWN).length === 0 && <span className="text-[#111111]/50">—</span>}</div>
                </div>
              </div>
              <div className="flex flex-wrap gap-2 text-[11px]">
                <span className="px-2 py-1 rounded-full bg-[#FFFDF9] border border-[#E8E1DA]">SPF · {label(skin.spfUsage)}</span>
                <span className="px-2 py-1 rounded-full bg-[#FFFDF9] border border-[#E8E1DA]">Soleil · {label(skin.sunExposure)}</span>
                <span className="px-2 py-1 rounded-full bg-[#FFFDF9] border border-[#E8E1DA]">Sensible · {label(skin.sensitivity)}</span>
              ...
              <div className="flex gap-2">
                <a href="/peau/routine" className="px-4 py-2 rounded-full bg-[#111111] text-white text-xs font-bold hover:bg-black">Voir ma routine</a>
                <a href="/boutique?cat=peau" className="px-4 py-2 rounded-full bg-white border border-[#E8E1DA] text-xs font-bold hover:border-[#C8753D]">Boutique peau</a>
              </div>
            </>
          )}
        </div>
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-[#FFFDF9] border border-[#E8E1DA] space-y-6">
          ...
          <div className="p-4 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA] space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button key={o.id} onClick={() => setFeeling(o.id)} className={`p-3 rounded-2xl border text-xs font-semibold ${feeling === o.id ? 'bg-[#C8753D] text-white border-[#C8753D]' : 'bg-white border-[#E8E1DA] hover:border-[#C8753D]'}`}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {SKIN_CONCERNS_SHORT.slice(0, 12).map(c => (
                  <button key={c.id} onClick={() => toggleJConcern(c.id)} className={`px-3 py-1.5 rounded-full border text-xs font-semibold ${jConcerns.includes(c.id) ? 'bg-[#C8753D] text-white border-[#C8753D]' : 'bg-white border-[#E8E1DA] hover:border-[#C8753D]'}`}>{c.label}</button>
                ))}
              </div>
              <p className="text-[11px] text-[#111111]/50 mt-1">{jConcerns.length}/3</p>
            </div>
            <div>
              <label className="text-xs font-semibold">Note (facultatif, 280 max)</label>
              <textarea value={jNotes} onChange={e => setJNotes(e.target.value)} maxLength={280} rows={3} placeholder="Ex: SPF réappliqué à midi, masque hydratant ce soir..." className="mt-1 w-full p-3 rounded-xl bg-white border border-[#E8E1DA] text-xs focus:outline-none focus:border-[#C8753D] resize-none" />
              <p className="text-[11px] text-[#111111]/40 text-right">{jNotes.length}/280</p>
            </div>
            <button onClick={addJournalEntry} disabled={saving} className="w-full py-3 rounded-full bg-[#C8753D] hover:bg-[#b06330] text-white text-xs font-bold flex items-center justify-center gap-1.5 disabled:opacity-60">
          ...
          <div className="space-y-3">
            {journal.length === 0 ? (
              <p className="text-xs text-[#111111]/50 text-center py-6">Aucune entrée pour l’instant — ajoutez votre première note ci-dessus.</p>
            ) : (
              journal.slice(0, 20).map((entry, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-[#F8F2EC] border border-[#E8E1DA] flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap gap-1 mb-1">{(entry.concerns || []).filter(v => v !== UNKNOWN).slice(0, 4).map(v => <span key={v} className="px-2 py-0.5 rounded-full bg-white border border-[#E8E1DA] text-[11px] font-semibold">{label(v)}</span>)}</div>
                    {entry.notes && <p className="text-xs text-[#111111]/70 leading-relaxed break-words">{entry.notes}</p>}
                  </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={saveProfile} disabled={saving} className="flex-1 py-3 rounded-full bg-[#111111] hover:bg-black text-white text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-60">
              {saving ? 'Enregistrement…' : <><Save className="w-4 h-4" /> Enregistrer Ma peau</>}
            </button>
            <a href="/peau/diagnostic/resultats" className="px-5 py-3 rounded-full border border-[#E8E1DA] text-xs font-semibold text-center hover:border-[#C8753D]">Voir le dernier résultat</a>
          </div>
          <p className="text-[11px] text-[#111111]/45 leading-relaxed flex gap-1.5"><AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> Journal beauté : suivi déclaratif, non médical. Si rougeurs, douleurs ou marques persistent, consultez un dermatologue. Export/suppression dans “Vos données”.</p>
        </div>

        {/* Liens peau */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <a href="/peau" className="p-4 rounded-2xl bg-[#FFFDF9] border border-[#E8E1DA] hover:border-[#C8753D]"><p className="font-bold flex items-center gap-1.5"><Zap className="w-4 h-4 text-[#C8753D]" /> Pôle Peau</p><p className="text-[#111111]/60 font-light mt-1">Explorer les 15 besoins & le guide HPI/SPF.</p></a>
          <a href="/peau/routine" className="p-4 rounded-2xl bg-[#FFFDF9] border border-[#E8E1DA] hover:border-[#C8753D]"><p className="font-bold flex items-center gap-1.5"><Clock className="w-4 h-4 text-[#C8753D]" /> Ma routine</p><p className="text-[#111111]/60 font-light mt-1">Matin / soir / hebdo, budget total.</p></a>
          <a href="/boutique?cat=peau" className="p-4 rounded-2xl bg-[#FFFDF9] border border-[#E8E1DA] hover:border-[#C8753D]"><p className="font-bold flex items-center gap-1.5"><FlaskConical className="w-4 h-4 text-[#C8753D]" /> Catalogue peau</p><p className="text-[#111111]/60 font-light mt-1">Filtrer par besoin, actif, budget.</p></a>
        </div>
      </div>
    </div>
  );
