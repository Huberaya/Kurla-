...
const STATUS_LABELS: Record<Appointment['status'], { label: string; className: string }> = {
  ...
  completed: { label: 'Réalisé', className: 'bg-kurla-copper/20 text-kurla-amber border-kurla-copper/40' },
  cancelled_by_client: { label: 'Annulé par le client', className: 'bg-white/5 text-kurla-cream/50 border-kurla-cream/15' },
  cancelled_by_pro: { label: 'Annulé par vous', className: 'bg-white/5 text-kurla-cream/50 border-kurla-cream/15' },
  ...
  if (!token) {
    return (
      <div className="min-h-screen pt-32 pb-24 bg-kurla-ink text-kurla-cream flex items-center justify-center px-4">
        <p className="text-sm text-kurla-cream/60">Connectez-vous pour accéder à votre espace professionnel.</p>
      </div>
    );
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-32 pb-24 bg-kurla-ink text-kurla-cream flex items-center justify-center">
        <p className="flex items-center gap-2 text-sm text-kurla-cream/60">
    ...
    return (
      <div className="min-h-screen pt-32 pb-24 bg-kurla-ink text-kurla-cream">
        <div className="max-w-2xl mx-auto px-4">
          <div className="rounded-3xl border border-kurla-cream/10 bg-kurla-espresso p-8">
            <h1 className="text-xl font-serif-title font-bold mb-3">Espace professionnel</h1>
            <p className="flex items-start gap-2 text-sm text-kurla-cream/70">
              <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
              <span>{error || 'Aucun profil professionnel vérifié n’est associé à ce compte.'}</span>
            </p>
            <p className="text-xs text-kurla-cream/50 mt-4 leading-relaxed">
            ...
            <a href="/kurla-pro" className="mt-6 inline-flex items-center gap-2 rounded-full bg-kurla-copper px-5 py-2.5 text-xs font-semibold text-white">
  ...
  return (
    <div className="min-h-screen pt-32 pb-24 bg-kurla-ink text-kurla-cream">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="p-8 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h1 className="text-2xl sm:text-3xl font-serif-title font-bold">{profile.displayName}</h1>
              {profile.identityVerified && (
                <span className="px-3 py-0.5 rounded-full bg-kurla-copper/20 text-kurla-amber text-xs font-semibold border border-kurla-copper/40 flex items-center gap-1">
          ...
          <a
            ...
            className="px-5 py-2.5 rounded-full bg-kurla-copper hover:bg-kurla-cocoa text-white text-xs font-semibold flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          <div className="p-6 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-2">
            <span className="text-xs text-kurla-amber font-semibold flex items-center gap-1.5">
          ...
          <div className="p-6 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-2">
            ...
            {trust.publishable && trust.score !== null ? (
              <>
                <span className="text-3xl font-bold">{trust.score}<span className="text-base text-kurla-cream/50">/100</span></span>
                <p className="text-[11px] text-kurla-cream/50">
              ...
              <>
                <span className="text-xl font-bold text-kurla-cream/60">Non publiable</span>
                <p className="text-[11px] text-kurla-cream/50">
        ...
        {trust.limitations.length > 0 && (
          <div className="mb-8 rounded-2xl border border-kurla-cream/10 bg-kurla-espresso/60 p-4">
            <p className="text-[11px] uppercase tracking-wider text-kurla-amber font-bold mb-2">Ce que ce score ne dit pas</p>
            <ul className="space-y-1">
              {trust.limitations.map((limitation, index) => (
                <li key={index} className="text-xs text-kurla-cream/60 leading-relaxed">· {limitation}</li>
              ))}
            </ul>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="p-8 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-4">
            ...
            {services.length === 0 ? (
              ...
              <ul className="space-y-2">
                {services.map(service => (
                  <li key={service.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold truncate">{service.name}</p>
                      <p className="text-[11px] text-kurla-cream/50 mt-0.5">
                        {service.durationMinutes} min{service.isRemote ? ' · à distance' : ' · en salon'}
                        {!service.isActive && ' · désactivée'}
                      </p>
                    </div>
                    <span className="text-sm font-bold text-kurla-amber shrink-0">
          ...
          <div className="p-8 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-4">
            ...
            {appointments.length === 0 ? (
              <p className="text-xs text-kurla-cream/55">Aucune réservation pour le moment.</p>
            ) : (
              <ul className="space-y-3">
                {appointments.slice(0, 8).map(appointment => {
                  const status = STATUS_LABELS[appointment.status];
                  return (
                    <li key={appointment.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-kurla-amber capitalize">{formatDateTime(appointment.scheduledAt)}</p>
                        <p className="text-[11px] text-kurla-cream/50 mt-1 flex items-center gap-1">
                        ...
                        {appointment.clientNotes && (
                          <p className="text-[11px] text-kurla-cream/45 mt-1 italic">« {appointment.clientNotes} »</p>
                        )}
                      </div>
        ...
        <div className="mt-6 p-8 rounded-3xl bg-kurla-espresso border border-kurla-cream/10 space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-serif-title font-bold flex items-center gap-2">
                <Users className="w-5 h-5 text-kurla-amber" /> Dossiers clients partagés
              </h2>
          ...
          {shares.length === 0 ? (
            ...
            <ul className="space-y-2">
              {shares.map(share => (
                <li key={share.id} className="p-4 rounded-2xl bg-kurla-ink border border-kurla-cream/5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                          ...
                          <span
                            ...
                            className={`rounded-full border px-2 py-0.5 text-[10px] ${
                              share[scope.key]
                                ...
                                : 'border-kurla-cream/10 text-kurla-cream/35'
                            }`}
                          >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-full bg-kurla-copper hover:bg-kurla-cocoa text-white text-[11px] font-semibold disabled:opacity-40 shrink-0"
                    >
          ...
          <div className="mt-6 pt-6 border-t border-kurla-cream/10">
            <h3 className="text-base font-serif-title font-bold mb-1">Co-signer une routine</h3>
            <p className="text-xs text-kurla-cream/55 mb-4 max-w-2xl">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <select
                    ...
                    className="px-3 py-2.5 rounded-xl bg-kurla-ink border border-kurla-cream/15 text-xs"
                  >
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2.5 rounded-xl bg-kurla-ink border border-kurla-cream/15 text-xs resize-none"
                />
                <label className="flex items-start gap-2 text-[11px] text-kurla-cream/60">
                  <input
                    type="checkbox"
                    checked={formConsent}
                ...
                <button
                  ...
                  className="px-4 py-2.5 rounded-full bg-kurla-copper hover:bg-kurla-cocoa text-white text-xs font-semibold disabled:opacity-40"
                >
          ...
          {dossier && (
            <div className="mt-4 rounded-2xl border border-kurla-copper/30 bg-kurla-copper/5 p-4">
              {!dossier.access ? (
                <p className="text-xs text-kurla-cream/70">{dossier.reason}</p>
              ) : (
                <>
                  <p className="text-[11px] uppercase tracking-wider text-kurla-amber font-bold mb-3">
                    Périmètre autorisé
                  </p>
                  <dl className="space-y-3">
                    {Object.entries(dossier.data || {}).map(([key, value]) => (
                      <div key={key}>
                        <dt className="text-xs font-semibold text-kurla-cream">{key}</dt>
                        <dd className="text-[11px] text-kurla-cream/60 mt-1 break-words">
                          {value === null
                  ...
                  <p className="mt-4 text-[10px] text-kurla-cream/45 leading-relaxed">{dossier.note}</p>
                </>
              )}
            </div>
          )}
