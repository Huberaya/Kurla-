      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {pillars.map((pillar, index) => (
            <div key={pillar.title} className="p-6 rounded-3xl bg-kurla-ivory border border-kurla-stone shadow-xs">
              <span className="text-[10px] uppercase font-bold text-kurla-copper block mb-1">
                {t('pages.melaninSkin.pillar')} {index + 1}
              </span>
              <h3 className="text-base font-bold text-kurla-carbon mb-2">{pillar.title}</h3>
              <p className="text-xs text-kurla-carbon/70 font-light leading-relaxed">{pillar.body}</p>
            </div>
          ))}
        </div>

        {/* Dermatologist Guardrail Disclaimer */}
        <div className="p-5 rounded-2xl bg-kurla-ivory border border-kurla-stone text-xs text-kurla-carbon/70 flex items-start gap-3 shadow-xs">
          <AlertCircle className="w-5 h-5 text-kurla-copper shrink-0 mt-0.5" />
          <div>
            <p className="font-bold text-kurla-carbon mb-0.5">{t('pages.melaninSkin.disclaimerTitle')}</p>
