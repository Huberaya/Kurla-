...
async function runScienceHubTests(): Promise<void> {
  ...
  assert.equal(allProblemCards.length, 21, '21 cartes moyens : 13 peau + 8 cheveux');
  const problemKeys = new Set(allProblemCards.map(card => card.key));
  assert.equal(problemKeys.size, 21, 'doublon de clé dans les cartes moyens');
  ...
  check('cartes moyens : 21 protocoles complets et sourcés (faire ≥ 3, éviter ≥ 2, délai honnête)');
  ...
  // --- 17h. Vague de fin : cernes, picking, prévention, teinte, saisonnier ---
  assert.ok(pickSkinProblemCards({ skinConcerns: ['cernes'] } as SkinAdvisoryContext).some(c => c.key === 'prob_skin_cernes'), 'cernes déclarés : la carte cernes doit être choisie');
  assert.ok(pickSkinProblemCards({ skinConcerns: ['picking'] } as SkinAdvisoryContext).some(c => c.key === 'prob_skin_picking'), 'picking déclaré : la carte picking doit être choisie');
  assert.ok(pickSkinScienceInsights({ skinObjectives: ['prevenir_age'] } as SkinAdvisoryContext).some(item => item.key === 'sci_skin_prevention'), 'objectif prévenir : la carte prévention doit être glissée');
  const maquillageTeinte = pickSkinScienceInsights({ skinConcerns: ['port_maquillage'] } as SkinAdvisoryContext);
  ...
  console.log(
    `[PASS] Base de savoirs (ouvrir les yeux) : ${HAIR_SCIENCE_CARDS.length} cartes cheveux + ${SKIN_SCIENCE_CARDS.length} cartes peau, toutes sourcées ; 9 thèmes sans orphelins ; 0 mot médical, 0 phrase réservée, corps peau sans mot interdit ; pickers déterministes bornés et contextuels ; wiring modèle de résultat (2 pôles) ; 21 cartes « moyens » (faire/éviter/s'attendre) sourcées, 4 pôles creusés (enfants, barbe, défrisage, locks), inconnu = inconnu — ${checks.length} checks.`
  );
}
