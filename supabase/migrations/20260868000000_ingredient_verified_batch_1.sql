...
INSERT INTO public.ingredients (id, inci_name, inci_name_normalized, common_names, functions, family, origin, verification_status, updated_at) VALUES ('salicylic_acid', 'Salicylic Acid', 'salicylic acid', ARRAY['acide salicylique', 'bha'], '{}', 'acides', 'synthèse ou végétal', 'verified', NOW())
ON CONFLICT (id) DO UPDATE SET
  inci_name = EXCLUDED.inci_name,
  inci_name_normalized = EXCLUDED.inci_name_normalized,
  common_names = EXCLUDED.common_names,
...
INSERT INTO public.ingredient_provenance (ingredient_id, source_label, source_url, retrieved_at, cas_number, evidence_tier, note) VALUES ('salicylic_acid', 'PubChem (NIH/NLM) — INCI présent dans la liste de synonymes, numéro CAS publié', 'https://pubchem.ncbi.nlm.nih.gov/compound/338', '2026-08-28', '69-72-7', 1, 'INCI vérifié : salicylic acid') ON CONFLICT (ingredient_id, source_url) DO UPDATE SET retrieved_at = EXCLUDED.retrieved_at, cas_number = EXCLUDED.cas_number, note = EXCLUDED.note;
